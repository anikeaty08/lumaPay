...
export function readDeploymentSecrets() {
    ...
    if (Boolean(mnemonic) === Boolean(seed)) {
        throw new Error('Supply exactly one of MIDNIGHT_WALLET_MNEMONIC or MIDNIGHT_WALLET_SEED.');
    }
    if (seed && !HEX_32_BYTES.test(seed)) {
        throw new Error('MIDNIGHT_WALLET_SEED must be exactly 32 bytes of hexadecimal.');
    }
    const admin = firstSet('LUMAPAY_REGISTRY_ADMIN_SECRET_HEX', 
    ...
    if (!admin || !HEX_32_BYTES.test(admin.value)) {
        throw new Error('LUMAPAY_REGISTRY_ADMIN_SECRET_HEX must be exactly 32 bytes of hexadecimal.');
    }
    const storage = firstSet('LUMAPAY_MIDNIGHT_PRIVATE_STATE_PASSWORD', 'MIDNIGHT_PRIVATE_STATE_PASSWORD', 
    // Temporary migration alias for credentials created before the LumaPay rename.
