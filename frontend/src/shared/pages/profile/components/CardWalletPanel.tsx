...
const TOKEN_OPTIONS: Array<{
    ...
    muted: string;
        ...
        { code: 'NIGHT', label: 'NIGHT', key: 'credits', tint: 'text-orange-400', muted: 'text-orange-300' }
    ];
...
const CARD_SAFETY_NOTES = [
    {
        ...
        body: 'LumaPay stores encrypted card payloads and metadata, not a live wallet someone can immediately use. The full card number, card address, and card private key are all stored as ciphertext.'
    },
    {
    ...
    },
    {
        ...
        body: 'Even when you pay with the card, decryption and signing happen inside your browser session after you enter the correct credentials. LumaPay only receives the signed authorization or encrypted payloads needed to relay the transaction.'
    },
    {
        title: 'Limits still reduce blast radius',
        body: 'The NIGHT card cap is anchored by the Midnight card-vault contract, and increasing that cap requires a fresh approval from your main wallet.'
    }
];
...
export const CardWalletPanel: React.FC<CardWalletPanelProps> = ({ itemVariants }) => {
    ...
    const [selectedTopUpToken, setSelectedTopUpToken] = useState<CardTokenCode>('NIGHT');
    const [topUpAmount, setTopUpAmount] = useState('');
    const [isTopUpPending, setIsTopUpPending] = useState(false);
    const [isUnlocking, setIsUnlocking] = useState(false);
    const [isInitializing, setIsInitializing] = useState(false);
    const [limitDrafts, setLimitDrafts] = useState<Partial<Record<CardTokenCode, string>>>({
        NIGHT: ''
    });
    ...
    const balances = {
        credits: cardBalances?.NIGHT || 0
    };

    const balanceCaps = {
        credits: card?.limits?.NIGHT?.max_balance || 0
    };
    ...
    const deleteGuidance = hasCardBalance
        ...
        : 'There is no balance in your LumaPay card. You can delete it from on-chain and off-chain both.';
    ...
    const handleCreate = async (event: React.FormEvent) => {
        ...
        try {
            ...
            toast.success('LumaPay Card created and published on-chain');
            setLabel('');
            setHint('');
            setPin('');
            setSecret('');
    ...
    const handleDeleteCard = async () => {
        ...
        try {
            ...
            await deleteCard();
            toast.success('LumaPay Card deleted successfully.');
        } catch (err: any) {
    ...
    const handleRefreshCardScan = async () => {
        try {
            ...
            toast.success('LumaPay card records refreshed.');
        } catch (err: any) {
        ...
        return (
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.15fr_0.85fr]">
                <GlassCard variants={itemVariants} className="p-6">
                    <div className="mb-6">
                        <span className="mb-3 block text-[10px] font-bold uppercase tracking-widest text-orange-400">LumaPay Card</span>
                ...
                <motion.div variants={itemVariants} className="grid grid-cols-1 gap-4 sm:grid-cols-3 xl:grid-cols-1">
                    ...
                    <GlassCard className="p-6 hover:border-white/20">
                        ...
                        <p className="text-sm leading-relaxed text-gray-500">
                            The NIGHT balance cap stays in place by default, and raising it remains a main-wallet approval flow.
                        </p>
                    </GlassCard>
                </motion.div>
            </div>
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 gap-4 xl:grid-cols-[1.15fr_0.85fr]">
                        ...
                        <div className="w-full max-w-[760px]">
                            ...
                            <div className="mt-4 flex flex-wrap items-center gap-3">
                                ...
                                <button
                                    ...
                                    title={hasCardBalance ? 'Sweep all private card funds back into the main wallet.' : 'This LumaPay card has zero balance.'}
                ...
                <div className="grid grid-cols-1 gap-4">
                    <GlassCard className="p-6">
                        ...
                        <h3 className="text-xl font-bold tracking-tighter text-gradient-gold drop-shadow-gold">Top up NIGHT on Preprod</h3>
                        <p className="mt-2 text-sm leading-relaxed text-gray-500">
                            Move only what you need onto the card. Preprod card funding is native NIGHT only.
                        </p>
                ...
                <GlassCard className="p-6 hover:border-white/20">
                    <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                        <div>
                            ...
                            <h3 className="mt-3 text-xl font-bold tracking-tighter text-gradient-gold drop-shadow-gold">Adjust NIGHT balance cap</h3>
                            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-gray-500">
                                Keep the active Preprod cap explicit and raise it only when you actually need more room on the card.
                            </p>
                        </div>
