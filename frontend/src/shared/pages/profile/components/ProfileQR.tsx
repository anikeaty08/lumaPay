...
export const ProfileQR: React.FC<ProfileQRProps> = ({ initialMainReceipts, initialBurnerReceipts }) => {
    const { initialized, loading, status, mainHash, mainPaymentUrl, burnerHash, burnerSalt, burnerPaymentUrl, initializeQRs } = useProfileQR();
    ...
    const paymentLink = (qrType === 'private' ? burnerPaymentUrl : mainPaymentUrl) || '';
                    ...
                    {unifiedPayments.length === 0 ? (
                        ...
                        unifiedPayments.map((payment, idx) => (
                            <div key={payment.receiptHash || idx} className="flex items-center justify-between bg-white/5 border border-white/5 rounded-xl p-3 hover:bg-white/10 transition-colors">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left">
                                        <div className="text-sm font-bold text-white">
                                            {payment.amount} {payment.tokenType === 0 ? 'NIGHT' : payment.tokenType === 1 ? 'USDCx' : 'USAD'}
                                        </div>
                                ...
                                {payment.txId && (
                                    <button
                                        onClick={() => { void navigator.clipboard.writeText(payment.txId); toast.success('Transaction ID copied.'); }}
                                        className="p-2 text-gray-500 hover:text-neon-primary transition-colors"
