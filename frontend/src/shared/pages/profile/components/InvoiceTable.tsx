...
const encodePaymentOpening = (opening: Record<string, unknown>) => {
    const bytes = new TextEncoder().encode(JSON.stringify(opening));
    let binary = '';
...
export const InvoiceTable: React.FC<InvoiceTableProps> = ({
    ...
    const renderTokenTotals = (
        ...
        const values = [
            { key: 'credits', amount: totals?.credits || 0, label: 'NIGHT' },
                ...
                <tbody className="divide-y divide-white/5">
                    {showShimmer ? (
                        ...
                        paginatedInvoices.map((inv, i) => {
                            ...
                            const canDelete = (inv.status === 'OPEN' || inv.status === 'PENDING') && !hasRecordedPayments;
                            ...
                            const deleteTitle = inv.walletType === 1 && !burnerDeleteReady
                                ...
                                : inv.invoiceType === 0
                                    ? 'Cancel or expire this invoice through Invoice Core'
                                    : 'Cancel or expire this campaign through the Campaigns contract';

                            const paymentLink = inv.paymentOpening
                                ? `${window.location.origin}/pay?opening=${encodeURIComponent(encodePaymentOpening(inv.paymentOpening))}`
                                : `${window.location.origin}/invoice/${encodeURIComponent(inv.invoiceHash)}`;
                            let paymentLinkNoMemo: string | undefined;
                            if (inv.paymentOpening?.memo) {
                                const openingWithoutMemo = { ...inv.paymentOpening };
                                delete openingWithoutMemo.memo;
                                paymentLinkNoMemo = `${window.location.origin}/pay?opening=${encodeURIComponent(encodePaymentOpening(openingWithoutMemo))}`;
                            }
                                ...
                                >
                                    ...
                                    <td className="py-4 px-6 text-center">
                                        <div className="flex flex-col gap-1 items-center">
                                            {inv.creationTx && (
                                                <button
                                                    onClick={(event) => { event.stopPropagation(); void navigator.clipboard.writeText(inv.creationTx); }}
                                                    className="text-[10px] text-gray-500 hover:text-neon-primary transition-colors underline decoration-dotted"
                                                    title="Copy creation transaction ID"
                                                >
                                                    Copy Create Tx
                                                </button>
                                            )}
                                    ...
                                    <td className="py-4 px-6 text-right">
                                        <div className="flex gap-2 justify-end w-full">
                                            ...
                                            {inv.status === 'SETTLED' && !inv.claimed && inv.claimMaterial && (
                                                <button
                                                    ...
                                                    title="Claim the settled escrow to your Midnight wallet"
                                                >
