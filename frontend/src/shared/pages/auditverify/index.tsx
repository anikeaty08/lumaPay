...
const AuditVerifyPage = () => {
    ...
    const aggregateRows = useMemo(() => {
        ...
        return [
            { token: 'NIGHT', earned: state.payload.summary.totalEarnings.credits.toFixed(2), outgoing: state.payload.summary.totalOutgoing.credits.toFixed(2) },
                                        ...
                                        {state.payload.invoices.map((invoice: AuditPayloadInvoice) => (
                                            <div key={invoice.invoiceHash} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
                                                ...
                                                <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                                                    ...
                                                    {invoice.creationTx && <InfoField label="Creation Tx" value={invoice.creationTx} mono copyId={`ac-${invoice.invoiceHash}`} copiedId={copiedId} onCopy={() => handleCopy(invoice.creationTx!, `ac-${invoice.invoiceHash}`)} explorerUrl={`https://api-preprod.1am.xyz/api/v4/graphql#transaction-${invoice.creationTx}`} />}
                                                </div>
                                                {invoice.paymentTxIds && invoice.paymentTxIds.length > 0 && (
                                                    <div className="mt-4">
                                                        ...
                                                        <div className="space-y-2">{invoice.paymentTxIds.map((txId: string, i: number) => <HashRow key={txId} value={txId} copied={copiedId === `ptx-${i}-${invoice.invoiceHash}`} onCopy={() => handleCopy(txId, `ptx-${i}-${invoice.invoiceHash}`)} explorerUrl={`https://api-preprod.1am.xyz/api/v4/graphql#transaction-${txId}`} />)}</div>
                                                    </div>
                                                )}
