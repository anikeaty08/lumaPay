...
import { WALLET_PROGRAM_ID } from '../../../utils/midnight/midnightUtils';
        ...
        try {
            ...
            for (const entry of nextEntries) {
                try {
                    ...
                    setEntries((current) => current.map((item) => (
                        item.id === entry.id
                            ? { ...item, balances: null, isLoadingBalances: false }
                            : item
                    )));
                }
            }
                                        ...
                                        <div key={token} className="px-1">
                                            ...
                                            {entry.isLoadingBalances ? (
                                                ...
                                                <p className={`mt-2.5 text-2xl font-bold tracking-tight transition-colors duration-300 ${
                                                    token === 'NIGHT' && (entry.balances?.[token] || 0) > 0
