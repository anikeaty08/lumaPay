...
/* ── token accent colors ── */
const TOKEN_CONFIG = {
    NIGHT:  { label: 'NIGHT',  color: '#F59E0B', glow: 'rgba(245,158,11,0.35)',  bg: 'rgba(245,158,11,0.07)'  },
} as const;
...
export const FloatingGiftCard: React.FC<FloatingGiftCardProps> = ({
    ...
    const hasValue = amounts && Number(amounts.NIGHT) > 0;
    const nonZeroTokens = amounts
        ? (['NIGHT'] as const).filter(t => Number(amounts[t]) > 0)
        : [];
