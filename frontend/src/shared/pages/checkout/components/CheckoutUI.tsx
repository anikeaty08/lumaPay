...
import { getTokenLabel, getTokenTypeFromCode } from '../../../utils/payments/tokens';
import { getUtf8ByteLength, COMPACT_PAYMENT_NOTE_MAX_BYTES } from '../../../utils/core/compactInputLimits';
import { looksLikeMidnightAddress, normalizeMidnightAddress } from '../../../utils/midnight/midnightAddress';
...
export const CheckoutUI: React.FC<CheckoutUIProps> = ({
    ...
    const [selectedPayerToken, setSelectedPayerToken] = useState<string>('NIGHT');
    ...
    const hasSelectableTokens = false;
        
    const displayToken = 'NIGHT';
    const displayTokenLabel = getTokenLabel(getTokenTypeFromCode('NIGHT'));

    useEffect(() => {
        if (!session) return;
        if (selectedPayerToken !== 'NIGHT') {
            setSelectedPayerToken('NIGHT');
        }
    }, [session, selectedPayerToken]);
    ...
    const paymentLink = typeof window !== 'undefined' && session ? (() => {
        ...
        const params = new URLSearchParams({
            ...
            salt: session.salt || '',
            type: invoiceTypeStr,
            token: (displayToken || 'night').toLowerCase(),
            ...
            >
                <div className="text-center mb-6">
                    <h1 className="text-3xl font-bold tracking-tighter text-white">
                        LumaPay <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-300 via-orange-500 to-amber-400 drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]">Checkout</span>
                    </h1>
                </div>
                                ...
                                <div className="rounded-2xl bg-black/30 p-4 space-y-4">
                                    <div>
                                        ...
                                        <p className={`mt-2 text-[11px] ${payerNoteTooLong ? 'text-red-400' : 'text-gray-500'}`}>
                                            Stored in one Compact field: {payerNoteBytes}/{COMPACT_PAYMENT_NOTE_MAX_BYTES} bytes.
                                        </p>
                                    </div>
                                            ...
                                            >
                                                ...
                                                <div className="flex justify-between items-center px-1">
                                                    <p className={`text-[11px] ${merchantNoteTooLong ? 'text-red-400' : 'text-gray-500'}`}>
                                                        {merchantNoteBytes}/{COMPACT_PAYMENT_NOTE_MAX_BYTES} bytes used
                                                    </p>
                                                </div>
                                            </motion.div>
                                        )}
                                ...
                                {paymentMethod === 'card' && (
                                    <div className="mb-4 animate-fade-in space-y-4 lg:grid lg:grid-cols-2 lg:gap-4">
                                        ...
                                        <p className="text-xs text-gray-500 text-center lg:col-span-2">
                                            The card key is decrypted in-memory on this device, used to generate the payment authorization locally, and never sent to LumaPay.
                                        </p>
