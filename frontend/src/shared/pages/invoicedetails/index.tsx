...
import { cancelInvoiceOnChain, expireInvoiceOnChain, type InvoiceRecoveryBundle } from '../../../midnight/contract';
...
const InvoiceDetailsPage: React.FC = () => {
    ...
    const { address, authenticated, api } = useWallet();
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                const data = (await fetchInvoices()).find((entry) => entry.invoice_hash === hash);
    ...
    const handleScanReceipts = async () => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/v1/chain/invoices/${encodeURIComponent(invoice.invoice_hash)}`);
            const chain = await response.json().catch(() => null);
            if (!response.ok) throw new Error(chain?.error?.message || 'Unable to read invoice receipts from Midnight.');
            const receiptHash = String(chain.receipt_commitment || '');
            const hasReceipt = chain.status === 'SETTLED' && receiptHash && !/^0+$/.test(receiptHash);
            setReceipts(hasReceipt ? [{
    ...
    const handleVerifyReceipt = async () => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/v1/chain/invoices/${encodeURIComponent(invoice.invoice_hash)}`);
            const chain = await response.json().catch(() => null);
            if (!response.ok) throw new Error(chain?.error?.message || 'Unable to verify receipt on Midnight.');
    ...
    const handleConfirmDelete = async () => {
        if (!invoice || !api) return;
        setIsDeleting(true);
        try {
            const raw = localStorage.getItem(`lumapay:recovery:${invoice.invoice_hash}`);
            if (!raw) throw new Error('Merchant recovery bundle is missing from this browser.');
            const recovery = JSON.parse(raw) as InvoiceRecoveryBundle;
            if (recovery.kind !== 'invoice' || recovery.invoiceId !== invoice.invoice_hash) {
                throw new Error('Merchant recovery bundle does not match this invoice.');
            }
            const expired = BigInt(invoice.expiry || recovery.expiry) <= BigInt(Math.floor(Date.now() / 1_000));
            const result = expired
                ? await expireInvoiceOnChain(api, invoice.invoice_hash)
                : await cancelInvoiceOnChain(api, recovery);
            const response = await fetch(`${API_URL}/api/v1/invoices/${encodeURIComponent(invoice.invoice_hash)}/reconcile`, {
                method: 'POST',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ transaction_id: result.transactionId }),
            });
            const body = await response.json().catch(() => null);
            if (!response.ok) throw new Error(body?.error?.message || 'Invoice reconciliation failed.');
            setShowDeleteModal(false);
            setInvoice((current) => current ? { ...current, status: expired ? 'EXPIRED' : 'CANCELLED' } : current);
        } catch (err) {
            handleWalletError(err);
            alert('Failed to update invoice: ' + (err instanceof Error ? err.message : 'Unknown error'));
        } finally {
            setIsDeleting(false);
        }
    };
    ...
    const handleCopyLink = () => {
        if (!invoice) return;
        if (invoice.payment_opening) {
            const bytes = new TextEncoder().encode(JSON.stringify(invoice.payment_opening));
            let binary = '';
            for (const byte of bytes) binary += String.fromCharCode(byte);
            const encoded = btoa(binary).replaceAll('+', '-').replaceAll('/', '_').replaceAll('=', '');
            navigator.clipboard.writeText(`${window.location.origin}/pay?opening=${encodeURIComponent(encoded)}`);
                    ...
                    <div className="space-y-3">
                        <div className="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-red-100/90">
                            ...
                            <p className="mt-2 leading-relaxed">
                                LumaPay submits a cancellation to Invoice Core. If its deadline has passed, the invoice is finalized as expired instead.
                            </p>
                        </div>
                        <p className="leading-relaxed text-gray-300">
                            This only works for an <span className="font-semibold text-white">open invoice with no settlement</span>.
                        </p>
                        <p className="leading-relaxed text-gray-300">
                            Your connected Midnight wallet must approve the transaction. LumaPay then reconciles the canonical indexed status.
                        </p>
                    </div>
                }
                confirmLabel="Confirm On-Chain Update"
            ...
            >
                <div className="max-w-screen-2xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {/* Cancel or expire only open invoices with no settlement. */}
                        {(invoice.status === 'OPEN' || invoice.status === 'PENDING') && (!invoice.payment_tx_ids || invoice.payment_tx_ids.length === 0) && (
                            <button
                                ...
                                title="Cancel or expire invoice on Midnight"
                            >
                                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <div className="flex flex-col gap-4">
                                            ...
                                            {totals.credits > 0 && (
                                                <div className="flex items-baseline gap-2">
                                                    ...
                                                    <span className="font-bold uppercase tracking-widest" style={{ fontSize: 'clamp(0.85rem, 2vw, 1.4rem)', color: 'rgba(0,243,255,0.75)' }}>NIGHT</span>
                                                </div>
                                            )}
                                            {totals.usdcx > 0 && (
                                                <div className="flex items-baseline gap-2">
