...
const toolCards = [
    {
        icon: KeyRound,
        title: 'login',
        description: 'Starts the LumaPay session, restores a backed-up password when possible, and can recover a burner wallet from on-chain records for the current session.'
    },
    {
        icon: Wrench,
        title: 'create_invoice',
        description: 'Creates a LumaPay invoice from the selected wallet and returns the payment link, invoice hash, and relayer-backed transaction details.'
    },
    {
        icon: Link2,
        title: 'pay_invoice',
        description: 'Accepts a LumaPay payment link or invoice hash, chooses the correct payment program, and pays the merchant from the user wallet.'
    },
    {
        icon: Search,
    ...
    return (
        <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
                ...
                <DeveloperStepCard step="03" title="Enter Wallet" desc="Provide your Midnight wallet address and optional local LumaPay password. Do not paste recovery phrases or private keys." icon={Wallet} />
                <DeveloperStepCard step="04" title="Restart Claude" desc="LumaPay writes the MCP config automatically, then Claude reloads the new local server." icon={RefreshCw} />
            </div>

            <GlassCard className="p-8 md:p-10">
                <span className="text-[11px] uppercase tracking-[0.25em] text-gray-500 font-semibold">MCP Setup</span>
                <h2 className="text-3xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-orange-300 to-orange-500 drop-shadow-[0_0_15px_rgba(249,115,22,0.3)] mt-3 mb-2">Install LumaPay MCP In A Few Minutes</h2>
                <p className="text-gray-400 text-sm leading-relaxed mb-8 max-w-4xl">
                    <code className="text-orange-200 bg-orange-500/10 border border-orange-400/15 px-1.5 py-0.5 rounded">@lumapay/mcp</code> lets Claude use LumaPay tools directly over MCP. The package already knows the LumaPay backend, so the user only needs wallet credentials. If the password was backed up in records, the setup can leave it blank and LumaPay can recover it during login.
                </p>

                <DeveloperCodeBlock
                    title="Install LumaPay MCP"
                    language="bash"
                    code={`npx -y @lumapay/mcp`}
                />

                <DeveloperCodeBlock
                    title="What The Setup Wizard Looks Like"
                    language="text"
                    code={`LumaPay MCP setup

Where do you want to install LumaPay MCP?
1. Claude Code (beta version)
2. Claude Desktop
3. Cancel
Choose 1, 2, or 3: 2

LumaPay will configure Claude automatically. You only need to provide your wallet credentials here.

Main wallet address: mn_...
LumaPay password (optional if you have it backed up in your records):

LumaPay MCP was added to your Claude config file.
Next step: fully restart Claude so it reloads the new MCP config.`}
                ...
                <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="rounded-2xl border border-orange-400/20 bg-orange-500/10 px-5 py-5">
                        ...
                        <p className="text-sm text-white/80 leading-relaxed">
                            Only the main wallet address and optionally the LumaPay password. Backend URLs and LumaPay-managed infrastructure settings are bundled into the package.
                        </p>
                    </div>
                ...
                <div className="mt-6 rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-5 py-5">
                    ...
                    <p className="text-sm text-white/85 leading-relaxed">
                        Do not enter a wallet recovery phrase or raw private key into AI clients. Wallet-approved actions should happen through the local Midnight wallet connector or a dedicated local operator process.
                    </p>
                </div>
            </GlassCard>
            ...
            <GlassCard className="p-8 md:p-10">
                ...
                <div className="space-y-4 text-sm text-gray-300 leading-relaxed">
                    <p><span className="text-white font-semibold">1.</span> Open a terminal and run <code className="text-white bg-white/5 px-1.5 py-0.5 rounded">npx -y @lumapay/mcp</code>.</p>
                    ...
                    <p><span className="text-white font-semibold">3.</span> Paste your wallet address. Enter the password only if you know it and want normal login to use it immediately.</p>
  ...
  "mcpServers": {
    "lumapay": {
      ...
      "env": {
        "LUMAPAY_MAIN_ADDRESS": "midnight1...",
        "LUMAPAY_MAIN_PASSWORD": "optional"
                ...
                <DeveloperCodeBlock
                    ...
                    language="bash"
                    code={`npx -y @lumapay/mcp server`}
                />
            </GlassCard>

            <GlassCard className="p-8 md:p-10">
                <span className="text-[11px] uppercase tracking-[0.25em] text-gray-500 font-semibold">Tooling</span>
                <h3 className="text-3xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-orange-300 to-orange-500 drop-shadow-[0_0_15px_rgba(249,115,22,0.3)] mt-3 mb-8">Available LumaPay MCP Tools</h3>
...
Start with login so the MCP can open the wallet session, recover on-chain backups when available, and decide whether the active wallet should be main or burner.`}
            ...
            <GlassCard className="p-8 md:p-10">
                ...
                <div className="space-y-4 text-sm text-gray-300 leading-relaxed">
                    <p>LumaPay MCP uses the production backend by default, so users do not need to manually enter the backend URL or public site URL.</p>
