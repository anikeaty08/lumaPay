        ...
        <div className="space-y-8">
            ...
            <GlassCard className="p-8 md:p-10">
                ...
                <p className="text-gray-400 text-sm leading-relaxed mb-8">
                    This is the same flow the demo merchant backend uses: your server creates the checkout session, the buyer is redirected to LumaPay hosted checkout, and your backend verifies settlement afterwards.
                </p>
                <div className="mb-8 rounded-2xl border border-orange-400/20 bg-orange-500/10 px-5 py-4 text-left">
                    ...
                    <p className="text-sm text-white/80 leading-relaxed">
                        LumaPay also has a relayer-backed setup path for invoices. In the CLI and Node SDK fallback flow, LumaPay submits the invoice-creation transaction on behalf of the merchant and covers that network fee from the relayer wallet.
                    </p>
                </div>

                <DeveloperCodeBlock
                    title="1. Install And Initialize"
                    code={`import { LumaPay } from '@lumapay/node';

const lumapay = new LumaPay({
  secretKey: process.env.LUMAPAY_SECRET_KEY!,
  baseURL: 'https://lumapay-backend-ib5q4.ondigitalocean.app/api', // optional override
  ...
  const session = await lumapay.checkout.sessions.retrieve(sessionId);
  ...
  res.send('Order confirmed');
            ...
            <GlassCard className="p-8 md:p-10">
                <h3 className="text-2xl font-bold text-gradient-gold drop-shadow-gold mb-3">Using lumapay.json (Pre-Generated Invoices)</h3>
                <p className="text-gray-400 text-sm leading-relaxed mb-6">
                    For multi-pay or donation-style flows, the best practice is to pre-generate your invoices and store them in a <code className="text-white bg-white/5 py-0.5 px-1.5 rounded">lumapay.json</code> file at the root of your backend project.
                    The <code className="text-neon-primary bg-white/5 py-0.5 px-1.5 rounded">@lumapay/node</code> SDK automatically reads this file!
                    There are two ways to create your <code className="text-white bg-white/5 py-0.5 px-1.5 rounded">lumapay.json</code>:
                </p>
                ...
                <div className="space-y-4 mb-8">
                    <div className="p-5 bg-white/[0.02] border border-white/[0.06] rounded-xl">
                        ...
                        <p className="text-gray-400 text-sm">Run our interactive CLI wizard: <code className="text-white bg-white/5 py-0.5 px-1.5 rounded">npx @lumapay/cli@latest sdk onboard</code>. It handles generating salts, submitting to the Midnight relayer, resolving the invoice hashes on the network, and writing the final <code className="text-white bg-white/5 py-0.5 px-1.5 rounded">lumapay.json</code> file for you. That invoice-creation step is relayed by LumaPay, so the merchant does not need to manually broadcast the setup transaction.</p>
                    </div>
                    <div className="p-5 bg-white/[0.02] border border-white/[0.06] rounded-xl">
                        <h4 className="text-gradient-gold drop-shadow-gold font-bold mb-2">2. Manual Fallback</h4>
                        <p className="text-gray-400 text-sm">If you don't want to use the CLI, you can manually interact with the LumaPay smart contract, generate your own hashes and salts, and create a <code className="text-white bg-white/5 py-0.5 px-1.5 rounded">lumapay.json</code> file matching the schema yourself.</p>
                    </div>
                </div>
                ...
                <DeveloperCodeBlock
                    title="Auto-fetching from lumapay.json"
                    code={`const session = await lumapay.checkout.sessions.create({
  lumapay_invoice_name: 'pro-plan', // 🚀 Auto-merges hash, salt, amount, and currency!
