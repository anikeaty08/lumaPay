...
export const SdkMethodsGuide: React.FC = () => {
        ...
        <div className="space-y-8">
            <GlassCard className="p-8 md:p-10">
                ...
                <div className="bg-black/40 rounded-2xl border border-white/[0.06] divide-y divide-white/[0.04] px-4 mb-8">
                    ...
                    <PropRow name="projectRoot" type="string" desc="Optional root folder used when resolving lumapay.json. Recommended on serverless runtimes." />
                    <PropRow name="configPath" type="string" desc="Optional exact file path to lumapay.json. Best option when you want deterministic lookup." />
                </div>

                <DeveloperCodeBlock
                    title="new LumaPay(config)"
                    code={`const lumapay = new LumaPay({
  ...
  configPath: path.join(__dirname, 'lumapay.json'),
                ...
                <DeveloperCodeBlock
                    title="lumapay.invoices (Local manifest helpers)"
                    code={`// Read the entire lumapay.json manifest into memory
const allInvoices = lumapay.invoices.getAll();

// Get a specific invoice by its name or index
const proPlan = lumapay.invoices.getByName('pro-plan');
const firstInvoice = lumapay.invoices.getByIndex(0);

// Filter your pre-generated invoices
const donationInvoices = lumapay.invoices.getByType('donation');`}
                />

                <p className="text-gray-500 text-xs leading-relaxed mt-4">
                    If you deploy on Vercel or another serverless runtime, prefer passing <code className="text-white bg-white/5 py-0.5 px-1.5 rounded">projectRoot</code> and <code className="text-white bg-white/5 py-0.5 px-1.5 rounded">configPath</code> so the SDK does not rely on the runtime working directory to find <code className="text-white bg-white/5 py-0.5 px-1.5 rounded">lumapay.json</code>.
                </p>

                <div className="bg-black/40 rounded-2xl border border-white/[0.06] divide-y divide-white/[0.04] px-4 mt-10">
                    <PropRow name="amount" type="number" desc="Amount in whole tokens. Intrinsically auto-resolved if utilizing lumapay_invoice_name." />
                    <PropRow name="currency" type="'NIGHT'" desc="Active Preprod token. Intrinsically auto-resolved if utilizing lumapay_invoice_name." />
                    <PropRow name="type" type="'standard' | 'multipay' | 'donation'" desc="Invoice mode. Intrinsically auto-resolved if utilizing lumapay_invoice_name." />
                    ...
                    <PropRow name="invoice_hash" type="string" desc="Zero-knowledge hash. Intrinsically auto-resolved if utilizing lumapay_invoice_name." />
                    <PropRow name="salt" type="string" desc="Private salt. Intrinsically auto-resolved if utilizing lumapay_invoice_name." />
                    <PropRow name="lumapay_invoice_name" type="string" desc="Shorthand. Automatically gets merged with amount, currency, invoice_hash, and salt from your local lumapay.json!" />
                </div>
...
console.log(session.checkout_url);`}
                ...
                <DeveloperCodeBlock
                    ...
                    code={`const session = await lumapay.checkout.sessions.retrieve('ses_abc123');
...
if (session.status === 'SETTLED') {
  ...
  console.log('Token:', session.token_type);
                ...
                <DeveloperCodeBlock
                    ...
                    code={`const isValid = lumapay.webhooks.verifySignature(rawBody, signature);

const event = lumapay.webhooks.constructEvent(rawBody, signature);`}
                />
            </GlassCard>
        </div>
    );
