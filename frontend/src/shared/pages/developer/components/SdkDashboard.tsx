...
} from '../../../utils/midnight/midnightUtils';
                ...
                {groupedReceipts.length === 0 ? (
                    ...
                    groupedReceipts.map(([invoiceHash, invoiceReceipts]) => {
                        ...
                        invoiceReceipts.forEach((receipt) => {
                            ...
                            if (receipt.tokenType === 0) totalNIGHT += amount;
                        });
                            ...
                            <tr key={invoiceHash} className="hover:bg-white/5 transition-colors group">
                                ...
                                <td className="py-5 px-6 text-center">
                                    <div className="flex flex-col items-center gap-1">
                                        {totalNIGHT > 0 && <span className="font-bold text-white text-md">{totalNIGHT} <span className="text-[10px] text-gray-500 uppercase">NIGHT</span></span>}
                                        {totalNIGHT === 0 && <span className="font-bold text-white/40 text-md">0 <span className="text-[10px] text-gray-500 uppercase">NIGHT</span></span>}
                                    </div>
                                </td>
    ...
    const handleSettle = async (invoice: SdkDashboardInvoice) => {
        ...
        try {
            ...
            const result = await executeWithShieldRetry(
                ...
                { onRetry: () => toast.loading('Midnight Wallet gave no response. Retrying settlement...', { id: 'shield-sdk-settle-retry' }) }
            );
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 gap-4">
                <SdkStatCard label="NIGHT Volume" value={sdkStats.credits} />
            </div>
