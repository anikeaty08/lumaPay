...
export const DeveloperPortal = () => {
    ...
    React.useEffect(() => {
        if (activeTab === 'analytics' && publicKey) {
            ...
            const apiUrl = import.meta.env.VITE_API_URL || 'https://lumapay-backend-ib5q4.ondigitalocean.app/api';
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto pt-8 pb-24 px-6">
                ...
                >
                    ...
                    <motion.p variants={fadeInUp} className="text-gray-400 text-lg md:text-xl max-w-3xl mx-auto leading-relaxed">
                        Merchant console for API keys, webhook setup, and SDK invoice operations. Full integration guides and technical reference now live at docs.lumapay.app.
                    </motion.p>
                    ...
                    <motion.div variants={fadeInUp} className="mt-8 mb-4 flex flex-col gap-4 items-center">
                        <div className="flex flex-wrap justify-center gap-3">
                            <div className="flex items-center gap-3 px-4 py-2 bg-white/[0.03] border border-white/[0.1] rounded-2xl hover:border-orange-500/30 hover:bg-white/[0.05] hover:shadow-[0_0_20px_rgba(249,115,22,0.05)] transition-all group max-w-fit cursor-pointer relative overflow-hidden" onClick={() => {
                                navigator.clipboard.writeText('npm install @lumapay/node@latest');
                                ...
                                <code className="text-sm font-mono text-orange-300 font-medium">npm install @lumapay/node@latest</code>
                            ...
                            <div className="flex items-center gap-3 px-4 py-2 bg-white/[0.03] border border-white/[0.1] rounded-2xl hover:border-blue-500/30 hover:bg-white/[0.05] hover:shadow-[0_0_20px_rgba(59,130,246,0.05)] transition-all group max-w-fit cursor-pointer relative overflow-hidden" onClick={() => {
                                navigator.clipboard.writeText('pip install lumapay-python');
                                ...
                                <code className="text-sm font-mono text-blue-300 font-medium">pip install lumapay-python</code>
                                <div className="flex items-center gap-2 ml-5 border-l border-white/5 pl-5">
                            ...
                            <div className="flex items-center gap-3 px-4 py-2 bg-white/[0.03] border border-white/[0.1] rounded-2xl hover:border-orange-500/30 hover:bg-white/[0.05] hover:shadow-[0_0_20px_rgba(249,115,22,0.05)] transition-all group max-w-fit cursor-pointer relative overflow-hidden" onClick={() => {
                                navigator.clipboard.writeText('npx @lumapay/cli@latest sdk onboard');
                                ...
                                <code className="text-sm font-mono text-orange-300 font-medium">npx @lumapay/cli@latest sdk onboard</code>
                                <div className="flex items-center gap-2 ml-5 border-l border-white/5 pl-5">
                            ...
                            <div className="flex items-center gap-3 px-4 py-2 bg-white/[0.03] border border-white/[0.1] rounded-2xl hover:border-orange-500/30 hover:bg-white/[0.05] hover:shadow-[0_0_20px_rgba(249,115,22,0.05)] transition-all group max-w-fit cursor-pointer relative overflow-hidden" onClick={() => {
                                navigator.clipboard.writeText('npx -y @lumapay/mcp');
                                ...
                                <code className="text-sm font-mono text-orange-300 font-medium">npx -y @lumapay/mcp</code>
                                <div className="flex items-center gap-2 ml-5 border-l border-white/5 pl-5">
                    ...
                    {activeTab === 'cli' && (
                        <motion.div key="cli" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            <GlassCard className="p-8 md:p-10">
                                <h2 className="text-3xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-orange-300 to-orange-500 drop-shadow-[0_0_15px_rgba(249,115,22,0.3)] mb-2">LumaPay CLI & lumapay.json</h2>
                                <p className="text-gray-300 text-sm leading-relaxed mb-10 max-w-3xl">
                                    The <code className="text-orange-200 bg-orange-500/10 border border-orange-400/15 px-1.5 py-0.5 rounded">@lumapay/cli</code> provides an interactive terminal wizard to securely configure and deploy your multi-pay and donation invoices directly to the Midnight network.
                                </p>
                                ...
                                <CodeBlock title="Run via npx" language="bash" code={`npx @lumapay/cli@latest sdk onboard`} />

                                <h3 className="text-lg font-bold text-gradient-gold drop-shadow-gold mb-4 mt-8">2. Review your lumapay.json</h3>
                                <p className="text-gray-500 text-sm mb-5 leading-relaxed">
                                    The CLI securely communicates with the Midnight testnet via the Relayer to deploy your invoices and returns a mapped <code className="text-orange-200 bg-orange-500/10 border border-orange-400/15 px-1.5 py-0.5 rounded">invoice_hash</code> and <code className="text-orange-200 bg-orange-500/10 border border-orange-400/15 px-1.5 py-0.5 rounded">salt</code> for each.
                                    These are saved in a <code className="text-white bg-white/5 border border-white/10 px-1.5 py-0.5 rounded">lumapay.json</code> file in your project root for your backend integration.
                                </p>
                                <CodeBlock title="Example lumapay.json" language="json" code={`{
  "merchant": "midnight1...",
  ...
  "invoices": [
    {
      ...
      "salt": "..."
                                ...
                                <p className="text-gray-500 text-sm mb-5 leading-relaxed">
                                    The <code className="text-orange-200 bg-orange-500/10 border border-orange-400/15 px-1.5 py-0.5 rounded">@lumapay/node</code> SDK automatically reads the <code className="text-white bg-white/5 border border-white/10 px-1.5 py-0.5 rounded">lumapay.json</code> file if it exists.
...
export async function createCheckout(req, res) {
    ...
    const session = await lumapay.checkout.sessions.create({
        lumapay_invoice_name: 'pro-plan', // Uses hash, salt, amount, and currency from lumapay.json
    ...
    res.redirect(303, session.checkout_url);
                                ...
                                <div className="mt-8 p-6 rounded-2xl bg-gradient-to-br from-orange-500/10 via-white/[0.02] to-transparent border border-orange-400/20 shadow-[0_0_30px_rgba(249,115,22,0.08)]">
                                    ...
                                    <p className="text-sm text-gray-300 leading-relaxed mt-4">
                                        If your backend runs on Vercel or another serverless runtime, initialize <code className="text-orange-200 bg-orange-500/10 border border-orange-400/15 px-1.5 py-0.5 rounded">LumaPay</code> with <code className="text-white bg-white/5 border border-white/10 px-1.5 py-0.5 rounded">projectRoot</code> and <code className="text-white bg-white/5 border border-white/10 px-1.5 py-0.5 rounded">configPath</code> so <code className="text-white bg-white/5 border border-white/10 px-1.5 py-0.5 rounded">lumapay.json</code> is resolved from the exact backend folder.
                                    </p>
                                </div>
                            </GlassCard>
                        </motion.div>
                                ...
                                {!publicKey ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06]">
                                            ...
                                            <p className="text-3xl font-black text-white">{stats.total_volume || 0} <span className="text-sm text-gray-400 font-medium">NIGHT</span></p>
                                        </div>
                                        <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06]">
                    ...
                    {activeTab === 'quickstart' && (
                        <motion.div key="quickstart" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                {[
                                    { n: '01', title: 'Install SDK', desc: 'One npm command to add the LumaPay Node client to your backend.', icon: Terminal },
                            ...
                            <GlassCard className="p-8 md:p-10">
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <a
                                            href="https://www.npmjs.com/package/@lumapay/node"
                                            target="_blank"
                                        ...
                                        <a
                                            href="https://pypi.org/project/lumapay-python/"
                                            target="_blank"
                                            rel="noopener noreferrer"
                                ...
                                <p className="text-gray-400 text-sm mb-6 leading-relaxed">
                                    LumaPay offers SDKs for Node.js (<code className="text-neon-primary bg-white/5 px-1.5 py-0.5 rounded">@lumapay/node</code>) and Python (<code className="text-blue-400 bg-white/5 px-1.5 py-0.5 rounded">lumapay-python</code>).
                                    Never expose your secret key on the frontend.
                                </p>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <CodeBlock title="Node.js" language="bash" code={`npm install @lumapay/node@latest`} />
                                    <CodeBlock title="Python" language="bash" code={`pip install lumapay-python`} />
                                </div>
...
export default lumapay;`} />
                                ...
                                <p className="text-gray-400 text-sm mb-6 leading-relaxed">
                                    ...
                                    hosted on LumaPay where the user's browser generates the ZK-proof.
                                </p>
                                <CodeBlock title="POST /api/checkout — Your backend route" code={`import lumapay from './lumapay';

export async function createCheckout(req, res) {
    const session = await lumapay.checkout.sessions.create({
        amount: 50,        // Number of NIGHT
        currency: 'NIGHT', // Preprod active token

        // The placeholder {CHECKOUT_SESSION_ID} is auto-replaced by LumaPay
        success_url: 'https://yoursite.com/success?session_id={CHECKOUT_SESSION_ID}',
        cancel_url: 'https://yoursite.com/cart',
    });

    // Redirect the user to the LumaPay payment page
    res.redirect(303, session.checkout_url);
}`} />
                                ...
                                <CodeBlock title="GET /success — Your success page handler" code={`import lumapay from './lumapay';

export async function handleSuccess(req, res) {
    const { session_id } = req.query;

    // Retrieve the session from LumaPay
    const session = await lumapay.checkout.sessions.retrieve(session_id);
                    ...
                    {activeTab === 'sdk' && (
                        <motion.div key="sdk" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            <GlassCard className="p-8 md:p-10">
                                ...
                                <h2 className="text-3xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-orange-300 to-orange-500 drop-shadow-[0_0_15px_rgba(249,115,22,0.3)] mt-3 mb-2">LumaPay Node SDK</h2>
                                <p className="text-gray-400 text-sm leading-relaxed mb-10">
                                    Complete reference for all methods in the <code className="text-neon-primary">@lumapay/node</code> package.
                                </p>
                                ...
                                <div className="space-y-12">
                                    <div>
                                        ...
                                        <p className="text-gray-500 text-sm mb-5 leading-relaxed">Constructor. Creates and authenticates the client with LumaPay servers.</p>
                                        <div className="bg-black/40 rounded-2xl border border-white/[0.06] divide-y divide-white/[0.04] px-4 mb-5">
                                            ...
                                            <PropRow name="projectRoot" type="string" desc="Optional backend root for resolving lumapay.json. Recommended on serverless runtimes." />
                                            <PropRow name="configPath" type="string" desc="Optional exact file path to lumapay.json for deterministic lookup." />
                                        </div>
    ...
    configPath: path.join(__dirname, 'lumapay.json'),
                                    ...
                                    <div className="pt-4 border-t border-white/[0.06]">
                                        <h3 className="text-xl font-bold text-white mb-1 flex items-center gap-2">
                                            ...
                                            lumapay.checkout.sessions.create(params)
                                        </h3>
                                        <p className="text-gray-500 text-sm mb-5 leading-relaxed">
                                        ...
                                        <div className="bg-black/40 rounded-2xl border border-white/[0.06] divide-y divide-white/[0.04] px-4 mb-5">
                                            <PropRow name="amount" type="number" required desc="The amount of NIGHT to charge. Denominated in whole units." />
                                            <PropRow name="currency" type="'NIGHT'" required desc="The active Preprod payment token." />
                                        ...
                                        <CodeBlock title="Create session" code={`const session = await lumapay.checkout.sessions.create({
    amount: 100,
    currency: 'NIGHT',
...
console.log(session.checkout_url);  // https://lumapay.app/checkout/ses_abc123`} />
                                    </div>

                                    <div className="pt-4 border-t border-white/[0.06]">
                                        <h3 className="text-xl font-bold text-white mb-1 flex items-center gap-2">
                                            <BookOpen className="w-5 h-5 text-gray-500" />
                                            lumapay.checkout.sessions.retrieve(id)
                                        </h3>
                                        <p className="text-gray-500 text-sm mb-5 leading-relaxed">
                                        ...
                                        <CodeBlock title="Retrieve session" code={`const session = await lumapay.checkout.sessions.retrieve('ses_abc123');
                        ...
                        <motion.div key="sessions" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            <GlassCard className="p-8 md:p-10">
                                ...
                                <div className="bg-black/40 rounded-2xl border border-white/[0.06] divide-y divide-white/[0.04] px-4 mb-10">
                                    ...
                                    <PropRow name="amount" type="number" desc="The amount charged, in whole NIGHT units. Available on retrieve." />
                                    <PropRow name="token_type" type="string" desc="Token type: NIGHT on Preprod. Available on retrieve." />
                                ...
                                <CodeBlock title="POST /api/checkout/sessions" language="http" code={`POST https://lumapay-backend-ib5q4.ondigitalocean.app/api/checkout/sessions
...
{
    "id": "ses_a1b2c3d4",
    "checkout_url": "https://lumapay.app/checkout/ses_a1b2c3d4",
    "status": "PROCESSING",
    "invoice_hash": "...",
    "salt": "..."
}`} />

                                <CodeBlock title="GET /api/checkout/sessions/:id" language="http" code={`GET https://lumapay-backend-ib5q4.ondigitalocean.app/api/checkout/sessions/ses_a1b2c3d4
Authorization: Bearer sk_test_xxxxxxxxxxxxxxxx
...
// Response 200 OK — payment settled
{
                    ...
                    {activeTab === 'webhooks' && (
                        <motion.div key="webhooks" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                            <GlassCard className="p-8 md:p-10">
                                ...
                                <p className="text-gray-400 text-sm leading-relaxed mb-10">
                                    Webhooks are the recommended way to fulfill orders. When a payment settles on-chain, LumaPay sends an
                                ...
                                <p className="text-gray-500 text-sm mb-5 leading-relaxed">
                                    LumaPay sends a JSON body containing the webhook event object. Use the SDK's <code className="text-neon-primary">constructEvent</code> to automatically parse and verify the HMAC-SHA256 signature in one step.
                                </p>
    ...
    "timestamp": "2025-03-13T07:30:00.000Z"
                                ...
                                <p className="text-gray-500 text-sm mb-5 leading-relaxed">
                                    ...
                                    If it doesn't, LumaPay will retry delivery up to 3 times with exponential backoff.
                                </p>
                                <CodeBlock title="Express webhook handler" code={`import express from 'express';
import lumapay from './lumapay';
...
app.post('/api/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    try {
        const event = lumapay.webhooks.constructEvent(req.body.toString(), signature);

        if (event.status === 'SETTLED') {
            // ✅ Double-check session status via API before fulfilling
            const verified = await lumapay.checkout.sessions.retrieve(event.id);
                                        ...
                                        <form onSubmit={handleRegister} className="space-y-5">
                                            ...
                                            <div>
                                                ...
                                                <input
                                                    ...
                                                    className="w-full bg-black/40 border border-white/[0.08] rounded-xl p-4 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-white/30 transition-colors font-mono"
                                                />
                                                <p className="text-[10px] text-gray-600 mt-1.5 ml-1">LumaPay will POST here every time a payment settles.</p>
                                            </div>
                                            <div>
                                    ...
                                    <div className="p-7 rounded-2xl bg-white/[0.02] border border-white/[0.06]">
                                        ...
                                        <div className="space-y-4">
                                            {[
                                                ...
                                                { label: 'One-Time Display', desc: 'The plaintext key is returned once — immediately after creation. After that, even LumaPay cannot recover it.' },
...
LUMAPAY_SECRET_KEY=sk_xxxxxxxxxxxxxxxx`} />
                                        <CodeBlock title="Loading in code" code={`import { LumaPay } from '@lumapay/node';
