...
export async function generateMerchantCreditReportPdf(input: CreditReportInput): Promise<void> {
    ...
    let y = addHeader(
        doc,
        'LumaPay Merchant Credit Report',
    ...
    runTable(doc, {
        ...
        body: [
            ...
            ['Main wallet credits volume', input.merchantStats.mainNIGHT],
            ['Burner wallet credits volume', input.merchantStats.burnerNIGHT]
        ],
        theme: 'plain',
        styles: { fillColor: CARD as [number, number, number], textColor: WHITE as [number, number, number], lineColor: [40, 44, 60] as [number, number, number], lineWidth: 0.2, fontSize: 8, cellPadding: 3.5 },
    ...
    addFooter(doc, 'LumaPay merchant credit report');
    doc.save(`LumaPay_Credit_Report_${shortHash(input.merchantAddress, 8, 6).replace('...', '_')}.pdf`);
}
...
export async function generateMerchantAuditReportPdf(input: AuditReportInput): Promise<void> {
    ...
    runTable(doc, {
        ...
        body: [
            ...
            ['Current mode', 'Wallet-visible records plus LumaPay backend invoice metadata']
        ],
        theme: 'plain',
        styles: { fillColor: CARD as [number, number, number], textColor: WHITE as [number, number, number], lineColor: [40, 44, 60] as [number, number, number], lineWidth: 0.2, fontSize: 8, cellPadding: 3.5 },
        headStyles: { fillColor: [28, 34, 50] as [number, number, number], textColor: ORANGE as [number, number, number], fontStyle: 'bold' },
    ...
    doc.setTextColor(...MUTED);
    doc.text(
        'This export packages the merchant-visible evidence currently available in LumaPay: wallet records, receipt hashes, invoice metadata, and transaction references. Chain-side selective disclosure packages and audit authorization anchors are not yet part of the current LumaPay contract.',
        18,
        y + 6,
        { maxWidth: pageWidth(doc) - 36 }
    );

    addFooter(doc, 'LumaPay merchant audit report');
    doc.save(`LumaPay_Audit_Report_${shortHash(input.merchantAddress, 8, 6).replace('...', '_')}.pdf`);
}
...
function explorerUrl(txId: string): string {
    return `https://api-preprod.1am.xyz/api/v4/graphql#transaction-${encodeURIComponent(txId)}`;
}
...
export function downloadMerchantCreditReportHtml(input: CreditReportInput, options?: ReportOptions): void {
    const opts = {
        filename: options?.filename || `LumaPay_Credit_Report_${shortHash(input.merchantAddress, 8, 6).replace('...', '_')}.html`,
    ...
    const hero = buildReportHero(
        ...
        'LumaPay',
        'Credit Report',
        'Interactive merchant credit report with the same dark glass-and-glow feel as the LumaPay dashboard. It highlights invoice activity, receipt coverage, memo visibility, and item-level detail for easy sharing.',
        [
...
<head>
    ...
    <title>LumaPay Credit Report</title>
...
export function buildMerchantAuditReportHtmlAsset(input: AuditReportInput, options?: ReportOptions): { html: string; filename: string } {
    ...
    const opts = {
        filename: options?.filename || `LumaPay_Audit_Report_${shortHash(input.merchantAddress, 8, 6).replace('...', '_')}.html`,
        includeMerchantAddress: options?.includeMerchantAddress !== false,
        includeBurnerAddress: options?.includeBurnerAddress !== false,
        includeMemo: options?.includeMemo !== false,
        includeLineItems: options?.includeLineItems !== false,
    ...
    const summaryCards = [
        ...
        ...(showMerchantView ? [{ label: 'Total NIGHT earned', value: units(totalEarnings.credits), compact: false }] : []),
        ...(showPayerView ? [{ label: 'Outgoing payer receipts', value: String(outgoingReceipts.length), compact: false }] : []),
        ...(showPayerView ? [{ label: 'Outgoing NIGHT', value: units(totalOutgoing.credits), compact: false }] : [])
    ].map((card) => `
        <div class="card">
    ...
    const hero = buildReportHero(
        ...
        'LumaPay',
        'Audit Report',
        'Detailed operational export for third-party review, styled to match the live LumaPay product. This report now highlights total earnings across all invoices so auditors can review the full picture without reading invoice totals one by one.',
        [
            { label: 'Export Type', value: 'Interactive HTML' },
...
<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>LumaPay Audit Report</title>
    <style>${reportThemeCss()}</style>
</head>
<body>
    <main>
        ...
        <section class="panel">
            ...
            <table>
                ...
                <tbody>
                    <tr><td>NIGHT</td>${showMerchantView ? `<td>${escapeHtml(units(totalEarnings.credits))}</td>` : ''}${showPayerView ? `<td>${escapeHtml(units(totalOutgoing.credits))}</td>` : ''}</tr>
                </tbody>
            </table>
        </section>
        ...
        <section class="panel note">
            <h2>Report Note</h2>
            <p>This export packages the merchant-visible evidence currently available in LumaPay: wallet records, receipt hashes, invoice metadata, and transaction references. Chain-side selective disclosure packages and audit authorization anchors are not yet part of the current LumaPay contract.</p>
        </section>
    </main>
    <script>
        (function () {
