...
function midnightSigningPayload(message: string): Uint8Array {
    return new TextEncoder().encode(`midnight_signed_message:${new TextEncoder().encode(message).byteLength}:${message}`);
}
...
export async function generateMerchantAuditPackage(
    ...
    signMessage?: (message: string) => Promise<{ signature: string; verifyingKey: string; signerAddress: string } | null>
): Promise<MerchantAuditBundle> {
    const filename = options?.filename || `LumaPay_Audit_Report_${input.merchantAddress.slice(0, 8)}.html`;
    ...
    const signatureMessage = `LUMAPAY_AUDIT_V1|${reportId}|${payload.role}|${generatedAt}|${payloadHash}|${auditKeyHash}`;
    const signatureResult = signMessage ? await signMessage(signatureMessage) : null;
    const auditPackage: MerchantAuditPackage = {
        version: '1.0.0',
        packageType: 'lumapay_audit_report',
        reportId,
        role: payload.role,
        filename,
        generatedAt,
        signerAddress: signatureResult?.signerAddress || input.merchantAddress,
        auditKeyHash,
        payloadHash,
        signatureMessage,
        signature: signatureResult?.signature || null,
        signatureBase64: null,
        verifyingKey: signatureResult?.verifyingKey || null,
...
export async function verifyMerchantAuditPackage(auditPackage: MerchantAuditPackage, auditKey: string): Promise<{ payload: MerchantAuditPayload; signatureStatus: 'verified' | 'missing' | 'invalid' }> {
    if (!auditPackage || auditPackage.packageType !== 'lumapay_audit_report') {
    ...
    if (auditPackage.signature && auditPackage.verifyingKey && auditPackage.signatureMessage) {
        try {
            const parsed = MidnightBech32m.parse(auditPackage.signerAddress);
            const expected = String(parsed.decode(UnshieldedAddress, parsed.network).hexString).toLowerCase();
            const actual = String(addressFromKey(auditPackage.verifyingKey)).toLowerCase();
            signatureStatus = expected === actual && verifySignature(
                auditPackage.verifyingKey,
                midnightSigningPayload(auditPackage.signatureMessage),
                auditPackage.signature
            ) ? 'verified' : 'invalid';
        } catch (error) {
