...
export function useProfileAggregations({
    ...
    const combinedInvoices = useMemo(() => {
        ...
        const processInvoice = (record: InvoiceRecord, walletType: number) => {
            ...
            merged.set(record.invoiceHash, {
                ...
                claimMaterial: record.claimMaterial ?? null,
                receiptCommitment: record.receiptCommitment,
                paymentOpening: record.paymentOpening
            });
        };
    ...
    const merchantStats = useMemo(() => {
        ...
        return {
            mainNIGHT: uniqueMainReceipts.reduce((acc, curr) => acc + (Number(curr.amount) / 1_000_000 || 0), 0).toFixed(2),
            // The first-wave contract suite settles NIGHT. Keep the historical
            // report shape explicit so generated receipts remain compatible.
            mainUSDCx: '0.00',
            mainUSAD: '0.00',
            burnerNIGHT: uniqueBurnerReceipts.reduce((acc, curr) => acc + (Number(curr.amount) / 1_000_000 || 0), 0).toFixed(2),
            burnerUSDCx: '0.00',
            burnerUSAD: '0.00',
            ...
            averageInvoiceAmountNIGHT: averageInvoiceAmountForToken(0),
