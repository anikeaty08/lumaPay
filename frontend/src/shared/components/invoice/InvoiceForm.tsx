...
import { getUtf8ByteLength, COMPACT_INVOICE_TITLE_MAX_BYTES, COMPACT_MEMO_MAX_BYTES } from '../../utils/core/compactInputLimits';
...
export const InvoiceForm: React.FC<InvoiceFormProps> = ({
    ...
    const invoiceTitleTooLong = invoiceTitleBytes > COMPACT_INVOICE_TITLE_MAX_BYTES;
    const memoBytes = getUtf8ByteLength(memo);
    const memoTooLong = memoBytes > COMPACT_MEMO_MAX_BYTES;
        ...
        <GlassCard variant="heavy" className="p-8">
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <div className="p-1.5 bg-black/40 rounded-2xl flex gap-1.5 border border-white/5 shadow-inner">
                        {[
                            { id: 0, label: 'Main Wallet', color: 'bg-white text-black', tooltip: 'Settles directly to your primary Midnight address. Simple and direct connection.' },
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-[10px] text-gray-500">Preprod payments currently settle in NIGHT.</p>
                        </div>
                                        ...
                                        <div className="relative z-10">
                                            <div className="flex items-center gap-3 mb-4">
                                                ...
                                                <div className="flex flex-col">
                                                    <h4 className="text-[13px] font-bold text-white tracking-tight leading-none">Native NIGHT Settlement</h4>
                                                </div>
                                            </div>

                                            <p className="text-[11px] leading-[1.6] text-gray-400 mb-5 font-medium">
                                                This Wave 1 build settles every payment directly in <span className="text-white font-bold underline decoration-orange-500/30 underline-offset-4">NIGHT</span>, finalized with <span className="text-white font-bold underline decoration-orange-500/30 underline-offset-4">ZK Proofs</span> for absolute privacy.
                                            </p>
                                            ...
                                            <div className="p-3.5 rounded-2xl bg-orange-500/[0.03] border border-orange-500/10 group-hover/tooltip:border-orange-500/20 transition-colors duration-500">
                                                <div className="flex items-center gap-2 mb-1.5">
                                                    <span className="text-[10px] text-orange-400 font-black uppercase tracking-[0.1em]">Cross-token Oracle</span>
                                                </div>
                                                <p className="text-[10px] text-gray-500 leading-relaxed">
                                                    Cross-token conversion is not live in this Preprod build. It will roll out once native stable-token contracts are deployed.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        {setSelectedAllowedTokens && invoiceType !== 'donation' ? (
                                            <div 
                                                onClick={() => {
                                                    if (isBase) return;
                                            ...
                                            <div className="px-3.5 pb-3 pt-3 border-t border-white/5 text-[10px] text-gray-500 font-medium leading-tight">
                                                Direct settlement in this token.
                                            </div>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="text-xs text-gray-400 text-center bg-white/5 p-3 rounded-lg border border-white/5">
                    ...
                    {invoiceType === 'donation' && (
                        <span>
                            ...
                            Used by NGOs, fundraising platforms, developers, and for crowd funding where the info of payer and receiver shall be kept private using records in Midnight.
                        </span>
                    )}
                </div>
                ...
                <div>
                    <Input
                        ...
                        value={invoiceTitle}
                        error={invoiceTitleTooLong ? `Keep title within ${COMPACT_INVOICE_TITLE_MAX_BYTES} bytes for a single Compact field.` : undefined}
                        onChange={(e) => setInvoiceTitle(e.target.value)}
                    />
                    <div className={`mt-2 text-xs ${invoiceTitleTooLong ? 'text-red-400' : 'text-gray-500'}`}>
                        This title is shared with the payer and stored in one Compact field: {invoiceTitleBytes}/{COMPACT_INVOICE_TITLE_MAX_BYTES} bytes.
                    </div>
                </div>
                ...
                <div>
                    <Input
                        ...
                        value={memo}
                        error={memoTooLong ? `Keep memo within ${COMPACT_MEMO_MAX_BYTES} bytes for a single Compact field.` : undefined}
                        onChange={(e) => setMemo(e.target.value)}
                    />
                    <div className={`mt-2 text-xs ${memoTooLong ? 'text-red-400' : 'text-gray-500'}`}>
                        Memo is optional to share and stored in one Compact field: {memoBytes}/{COMPACT_MEMO_MAX_BYTES} bytes. Regular letters usually count as 1 byte.
                    </div>
                </div>
