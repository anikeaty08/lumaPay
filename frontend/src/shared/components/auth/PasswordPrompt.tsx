...
import { getUtf8ByteLength, COMPACT_PASSWORD_BACKUP_MAX_BYTES } from '../../utils/core/compactInputLimits';
...
export const PasswordPrompt: React.FC<{ variant?: 'full' | 'compact' }> = ({ variant = 'full' }) => {
    ...
    const passwordTooLong = isCreatingPassword && passwordBytes > COMPACT_PASSWORD_BACKUP_MAX_BYTES;
    ...
    const details = isCreatingPassword
        ? [
            ...
            'LumaPay uses that encrypted layer to protect things like your private platform profile, burner wallet data, and card metadata before storing them.',
            'Without the password, the stored data stays unreadable, which helps protect you if someone only gets database access.',
            `For on-chain password backup compatibility, setup passwords should stay within ${COMPACT_PASSWORD_BACKUP_MAX_BYTES} bytes.`
        ]
        : [
            ...
            'This unlock step lets LumaPay restore protected items like burner wallet details, encrypted profile data, and card-related private metadata.',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isNewUser) {
            ...
            if (passwordTooLong) {
                toast.error(`Password is too large for the Leo backup field. Keep it within ${COMPACT_PASSWORD_BACKUP_MAX_BYTES} bytes.`);
                return;
            }
                ...
                <div className="flex flex-col items-center text-center">
                    ...
                    <form onSubmit={async (e) => {
                        ...
                        <p className={`text-left text-xs leading-relaxed ${passwordTooLong ? 'text-red-400' : 'text-white/55'}`}>
                            {passwordBytes}/{COMPACT_PASSWORD_BACKUP_MAX_BYTES} bytes
                        </p>
                        <button
                            type="submit"
                ...
                <div className="relative z-10 w-full max-w-md space-y-5">
                    <GlassCard className="w-full border border-orange-400/15 bg-[#080808]/85 p-8 text-center shadow-[0_0_60px_rgba(249,115,22,0.08)]">
                        ...
                        <p className="mb-6 text-sm leading-relaxed text-white/60">
                            Your account is from an older version of LumaPay and now needs a password to protect private app data.
                        </p>
                        <form onSubmit={async (e) => {
                            e.preventDefault();
                            if (!address || password.length < 6) {
                            ...
                            <p className={`text-left text-xs leading-relaxed ${passwordTooLong ? 'text-red-400' : 'text-white/55'}`}>
                                Setup passwords should fit one Leo backup field: {passwordBytes}/{COMPACT_PASSWORD_BACKUP_MAX_BYTES} bytes. Regular letters usually count as 1 byte.
                            </p>
                            <button
                                type="submit"
                                disabled={loading || passwordTooLong}
