...
const LumaPay_AI_CONTEXT = {
    about: `LumaPay is a privacy-first payment protocol built on Midnight (a zero-knowledge proof blockchain). It enables merchants to create invoices and accept payments without exposing transaction details on-chain. Invoice details are hashed with BHP256, payments run through private transfer paths, and both merchant and payer receive private receipts verified by zero-knowledge proofs. The platform includes: a hosted checkout UI, merchant backend (Node.js/Express), Node.js SDK (@lumapay/node), Python SDK (@lumapay/python), CLI onboarding tool (@lumapay/cli), MCP server for AI clients (@lumapay/mcp), and merchant dashboards.`,
    contracts: [
        'zk_pay_proofs_privacy_v29.midnight — Core payment contract (invoice creation, payment, settlement, Oracle quotes)',
        'zk_pay_proofs_privacy_wallet_v6.midnight — Wallet helper (burner backup, card profiles, gift cards, cross-token Oracle payments)',
    ],
    tokens: [
        'NIGHT — Midnight native token (gas)',
        'NIGHT-only Preprod settlement',
        'Native Midnight wallet approvals',
    ],
    links: {
        website: 'https://lumapay.app',
        github: 'https://github.com/LumaPay',
        midnightDocs: 'https://midnight.org/post/midnight-developer-documentation/',
        midnightSdk: 'https://github.com/MidnightHQ/sdk',
        midnightWalletAdaptor: 'https://github.com/MidnightHQ/midnight-wallet-adaptor',
        leoLanguage: 'https://leo-lang.org/',
        npmNode: 'https://www.npmjs.com/package/@lumapay/node',
        npmMcp: 'https://www.npmjs.com/package/@lumapay/mcp',
        npmCli: 'https://www.npmjs.com/package/@lumapay/cli',
...
const buildAIContext = (
    ...
    const linksEntries = Object.entries(LumaPay_AI_CONTEXT.links)
        .map(([k, v]) => `- ${k}: ${v}`)
        .join('\n');

    return [
        '# LumaPay — AI Context Package',
        '',
        '## About LumaPay',
        LumaPay_AI_CONTEXT.about,
        '',
        '## Smart Contracts',
        ...LumaPay_AI_CONTEXT.contracts.map((c) => `- ${c}`),
        '',
        '## Supported Tokens',
        ...LumaPay_AI_CONTEXT.tokens.map((t) => `- ${t}`),
        '',
        '## Key External Links',
        linksEntries,
        '',
        '## Full Documentation Index',
        'Every section in the LumaPay docs with its direct URL:',
        '',
        docIndex,
        '',
        '---',
