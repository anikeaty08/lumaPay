...
export const checkoutLifecycleSection: DocsSection = {
    ...
    summary:
        'A checkout in LumaPay is a stateful journey. From the initial merchant handshake to the final on-chain settlement, every step is designed to preserve privacy while ensuring deterministic fulfillment.',
    content: (
        <div className="space-y-6">
            <div className="grid gap-5 md:grid-cols-4">
                ...
                <MetricCard
                    icon={ShieldCheck}
                    title="Settlement"
                    description="The Midnight network verifies the proof and moves funds privately to the merchant's pool."
                />
                <MetricCard
                    icon={Truck}
                    title="Fulfillment"
            ...
            <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-blue-500/50 before:to-transparent">
                {[
                    ...
                    },
                    {
                        ...
                        icon: ShoppingCart,
                        color: 'text-blue-400',
                        desc: 'The buyer is redirected to https://lumapay.app/pay/{SESSION_ID}. Here, the LumaPay client retrieves the invoice details from the Midnight mapping to show the price and currency to the buyer.'
                    },
                    {
                        title: '3. ZK Proof Generation',
                        icon: Zap,
                        color: 'text-yellow-400',
                        desc: 'The buyer approves the transaction. The LumaPay wallet (or burner) generates a Zero-Knowledge proof locally. This proof hides the buyer\'s balance but proves they have enough funds to cover the invoice.'
                    },
                    {
                        title: '4. Chain Commitment',
                        icon: ShieldCheck,
                        color: 'text-emerald-400',
                        desc: 'The transaction is broadcast to the Midnight network. Once the block is finalized, the invoices mapping for that specific hash is updated from "Open" to "Settled" (for one-time invoices).'
                    },
                    {
                        title: '5. Webhook & Fulfillment',
                        icon: RefreshCw,
                        color: 'text-purple-400',
                        desc: 'The LumaPay indexers detect the mapping change. A signed HMAC-SHA256 webhook is sent to the merchant\'s backend. The merchant verifies the signature and completes the order.'
                    }
                ].map((step, i) => (
            ...
            <GlassCard className="p-6 border-emerald-500/20 bg-emerald-500/5">
                ...
                <div className="grid gap-4 md:grid-cols-3 text-xs text-gray-400">
                    ...
                    <div className="space-y-1">
                        ...
                        <p>~10s for Midnight block finality. Webhook fired within 2s of block.</p>
                    </div>
                </div>
            </GlassCard>
        </div>
