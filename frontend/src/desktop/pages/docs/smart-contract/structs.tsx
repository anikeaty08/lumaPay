 ...
 * request on the Midnight ledger. It contains the metadata 
...
struct InvoiceData {
    ...
    token_type:    u8,     // Currency: NIGHT(0) on active Preprod
    wallet_type:   u8      // Context: Main(0) | Burner(1)
}`;

const oracleQuoteStructExample = `/**
 * FUTURE QUOTE STATE: struct OracleQuote
 * 
 * This struct represents a price quote from the LumaPay Oracle.
 * Reserved for future quote/payment routes after native token
 * contracts are deployed and tested.
 */

struct OracleQuote {
    original_amount_micro:  u64,   // Amount in source token
    converted_amount_micro: u64,   // Amount in target token
    from_token_type:        u8,    // Source token type
    to_token_type:          u8,    // Target token type
...
export const structsSection: DocsSection = {
    ...
    summary:
        'Structs define the standard data formats for all public state transitions in LumaPay. They ensure that data is packed efficiently into Midnight field elements while remaining human-readable for developers.',
    content: (
        <div className="space-y-6">
            <div className="grid gap-5 md:grid-cols-4">
                ...
                <MetricCard
                    icon={Database}
                    title="Durable State"
                    description="Once written to a mapping, these structs form the immutable history of the LumaPay registry."
                />
            </div>
            ...
            <GlassCard className="overflow-hidden p-0 border-blue-500/20">
                ...
                <div className="px-6 py-5">
                    <p className="mb-4 text-sm text-gray-400">
                        Reserved for future quote metadata. The active Preprod build does not expose cross-token payment routes.
                    </p>
            ...
            <Callout title="A Note on Numeric Precision" tone="blue">
                In Midnight, structs are public. LumaPay stores only commitments and control flags in public state. Amount details stay in private wallet-approved openings or are re-derived during payment verification.
            </Callout>
        </div>
    ),
};
