 ...
 * 
 * This function "mints" a new payment request onto the Midnight ledger.
 * It is typically called by the LumaPay Relayer on behalf of the merchant
        ...
        <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
                <GlassCard className="p-6">
                    ...
                    <p className="text-sm text-gray-400 leading-relaxed">
                        In the LumaPay ecosystem, invoice creation is usually "sponsored". The CLI/SDK sends the parameters to our relayer, which then submits the <code className="text-white/80">create_invoice</code> transaction. This means the merchant doesn't need to hold Midnight NIGHT to start accepting payments.
                    </p>
                </GlassCard>
                <GlassCard className="p-6">
                    <div className="mb-4 flex items-center gap-2">
            ...
            <GlassCard className="p-6 bg-orange-500/5 border-orange-500/20">
                ...
                <p className="mb-4 text-sm text-gray-400 leading-relaxed">
                    LumaPay uses <b>Homomorphic Commitment</b> principles. By hashing each input part individually and then summing them, we allow for future protocol upgrades where "blinded amounts" or "merchant groups" can be verified without breaking the original hash format.
                </p>
            ...
            <Callout title="A Note on Expiry" tone="blue">
                Expiry is defined in <b>Hours</b> for developer convenience. Inside the finalizer, this is converted to Midnight block height: <br />
                <code className="mt-1 block bg-white/5 p-2 rounded text-emerald-300">target_height = current_height + (expiry_hours * 360)</code>
                <br />
                This assumes a 10-second block time (standard for Midnight Testnet).
            </Callout>
        </div>
    ),
};
