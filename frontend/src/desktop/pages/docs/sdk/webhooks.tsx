...
// Verify this with LumaPay.webhooks.constructEvent()
...
interface WebhookEvent {
    ...
    amount:     number;
    /** Token type: 'NIGHT' on Preprod */
    token_type: string;
    /** Payment status: SETTLED | FAILED | PROCESSING | PENDING */
    status:     'SETTLED' | 'FAILED' | 'PROCESSING' | 'PENDING';
    /** Midnight transaction ID (at1...). Null if payment not yet submitted. */
    tx_id:      string | null;
    /** Invoice name from your lumapay.json (e.g., 'basic-credits') */
    lumapay_invoice_name?: string;
    /** ISO 8601 timestamp */
    timestamp:  string;
}`;

const step1Setup = `/**
 * STEP 1: Initialize LumaPay with your Secret Key
 *
 * Get your secret key from the LumaPay Developer Portal:
 * https://lumapay.app/developer
 */
 ...
 *
 * Register this endpoint in LumaPay Developer Portal:
 * https://lumapay.app/developer
    ...
    try {
        ...
        if (event.status === 'SETTLED') {
            orderStatusStore.set(event.id, {
                ...
                invoiceName: event.lumapay_invoice_name || null,
                timestamp: Date.now()
            });
...
app.get('/api/order-status/:sessionId', (req, res) => {
    ...
    LumaPay.checkout.sessions.retrieve(sessionId)
        .then(session => {
            return res.json({
                ...
                invoiceName: session.lumapay_invoice_name || null
            });
        })
        .catch(err => {
...
const step5LumaPayJson = `/**
 * STEP 5: lumapay.json - Define your products/invoices
 */
 ...
 * RETRY POLICY
 *
 * If your server returns non-2xx, LumaPay retries:
 * Attempt 1: Immediate
 * Attempt 2: 5 minutes
...
export const webhooksSection: DocsSection = {
    ...
    summary:
        'A step-by-step guide to integrating LumaPay webhooks. Includes real working code from the testing website.',
    content: (
        <div className="space-y-6">
            ...
            <GlassCard className="p-6">
                <div className="mb-4 flex items-center gap-2">
                    ...
                    <h3 className="text-xl font-bold text-white">Initialize LumaPay SDK</h3>
                </div>
                <p className="mb-4 text-sm text-gray-400">
                    Set up Express with CORS and initialize LumaPay with your secret key from the developer portal.
                </p>
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-4 text-sm text-gray-400">
                    Create invoice definitions in lumapay.json.
                </p>
                <CodeBlock title="backend/lumapay.json" language="json" code={step5LumaPayJson} />
            </GlassCard>
            ...
            <GlassCard className="p-6">
                ...
                <Callout title="Idempotency is Mandatory" tone="blue">
                    Since LumaPay retries deliveries, your server might receive the same SETTLED event twice.
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-4 text-sm text-gray-400">
                    ...
                    The signature is in the <code className="text-white/80">x-lumapay-signature</code> header.
                </p>
