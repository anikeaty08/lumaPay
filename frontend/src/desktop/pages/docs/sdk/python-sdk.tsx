...
const installExample = `# Install the official LumaPay Python SDK
...
# Typically in a file like: dependencies.py or utils/LumaPay.py
...
LumaPay = LumaPay({
    ...
    "secret_key": os.getenv("LUMAPAY_SECRET_KEY"),
    ...
    "base_url": os.getenv("LUMAPAY_BASE_URL", "https://api.lumapay.xyz"),

    # OPTIONAL: directory where lumapay.json lives (defaults to Path.cwd())
    "project_root": str(Path(__file__).parent),

    # OPTIONAL: override the exact path to lumapay.json
    "config_path": str(Path(__file__).parent / "lumapay.json"),
...
export const pythonSdkSection: DocsSection = {
    ...
    summary:
        'The lumapay-python SDK provides a fully native Python interface to the LumaPay protocol. It bridges your backend to the Midnight blockchain without requiring Node.js, abstracting ZK-proof generation, relayer sponsorship, and polling into clean Python methods.',
    content: (
        <div className="space-y-6">
            ...
            <GlassCard className="p-6">
                ...
                <div className="mt-4 rounded-lg border border-white/[0.08] bg-white/[0.02] p-4">
                    ...
                    <ul className="mt-2 space-y-2 text-xs text-gray-400">
                        <li>• <code className="rounded bg-white/5 px-1 py-0.5">LumaPay</code>: The primary client class for all operations.</li>
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-4 text-sm text-gray-400">
                    The Python SDK uses a similar architecture to the Node SDK, allowing precise control over where the <code className="text-gray-300">lumapay.json</code> file is loaded from, avoiding common pathing issues in containerized environments.
                </p>
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                    <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-4">
                        ...
                        <p className="text-xs text-gray-400">Set this when your <code className="text-gray-300">lumapay.json</code> is located at the root of your application, and the SDK should search for it there.</p>
                    </div>
                    <div className="rounded-lg border border-purple-500/20 bg-purple-500/5 p-4">
                        <p className="mb-1 text-xs font-bold text-purple-300 uppercase">When to use config_path</p>
                        <p className="text-xs text-gray-400">Use this for absolute precision, directly pointing the SDK to the exact path of your <code className="text-gray-300">lumapay.json</code> file.</p>
                    </div>
                </div>
            </GlassCard>
            ...
            <GlassCard className="p-6">
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-3">
                    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4 transition-all hover:bg-white/[0.04]">
                        ...
                        <p className="text-xs leading-relaxed text-gray-400">Handles local manifest reading. It validates your <code className="text-white/80">lumapay.json</code> and allows simple dictionary lookups by name, type, or index.</p>
                    </div>
                    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4 transition-all hover:bg-white/[0.04]">
                        <div className="mb-2 flex items-center gap-2">
            ...
            <Callout title="Production Best Practice: Environment Variables" tone="blue">
                Always store your <code className="rounded bg-blue-500/20 px-1.5 py-0.5 text-blue-200">LUMAPAY_SECRET_KEY</code> securely. In Python, use <code className="text-blue-200">os.getenv("LUMAPAY_SECRET_KEY")</code> alongside packages like <code className="text-blue-200">python-dotenv</code>.
            </Callout>
        </div>
    ),
};
