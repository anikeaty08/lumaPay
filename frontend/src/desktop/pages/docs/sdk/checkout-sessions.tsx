...
// These types are exported from @lumapay/node
...
export interface CreateCheckoutSessionParams {
    ...
    /**
     * The token ticker. Preprod live execution is NIGHT-only.
     */
    currency?:  'NIGHT';  
    ...
    // ─── Shorthand (recommended for lumapay.json workflows) ─────────────
    lumapay_invoice_name?:  string;  // Read from lumapay.json by name
    LumaPay_invoice_index?: number;  // Read from lumapay.json by array index
}
...
// Recommended for 90% of use cases. Uses pre-configured invoices from lumapay.json.

const session = await LumaPay.checkout.sessions.create({
    lumapay_invoice_name: 'basic-monthly',
    success_url: 'https://yourapp.com/success?session_id={CHECKOUT_SESSION_ID}',
    cancel_url:  'https://yourapp.com/cancel',
});

// Implementation details:
// 1. SDK reads 'basic-monthly' from your local lumapay.json file.
// 2. Extracts merchant_address, hash, salt, and amount.
// 3. Submits to LumaPay Cloud APIs immediately.
 ...
 * 2. Relayer Request: Calls the LumaPay Relayer to sponsor the transaction.
 * 3. Gas Coverage: LumaPay pays the gas fee to create the invoice on Midnight.
 * 4. Polling Lifecycle:
 *    - The SDK enters a polling loop (2s intervals).
 *    - It queries the Midnight Mapping: salt_to_invoice[your_salt].
...
switch (session.status) {
    case 'SETTLED':
        ...
        // Payment is confirmed on the Midnight blockchain.
        break;
    case 'PROCESSING':
...
export const checkoutSessionsSection: DocsSection = {
    ...
    summary:
        'Checkout Sessions are the fundamental unit of commerce in LumaPay. They represent a single "Hosted Checkout" intent, linking a merchant invoice (on-chain) to a specific payment request (off-chain).',
    content: (
        <div className="space-y-6">
            <div className="grid gap-4 md:grid-cols-2">
                <GlassCard className="p-6">
                    ...
                    <p className="text-sm text-gray-400">
                        Sessions are linked to your <code className="text-white/80">invoice_hash</code>. This hash is a commitment on the Midnight blockchain that ensures nobody can modify the amount or recipient address once the session is created. The SDK verifies this commitment before generating the checkout URL.
                    </p>
                </GlassCard>
                <GlassCard className="p-6">
                    <div className="mb-4 flex items-center gap-2">
                    ...
                    <p className="text-sm text-gray-400">
                        The SDK uses the <code className="text-white/80">salt_to_invoice</code> mapping to verify that the invoice exists on-chain. This mapping is public, allowing the buyer to verify the authenticity of the payment request using the Midnight Explorer.
                    </p>
                </GlassCard>
            </div>
            ...
            <GlassCard className="overflow-hidden p-0">
                ...
                <div className="px-6 py-5">
                    <p className="mb-4 text-sm text-gray-400">
                        This is the high-performance path. By using <code className="text-emerald-300">lumapay_invoice_name</code>, the SDK skips all blockchain interaction. It assumes the invoice already exists on-chain (created via CLI) and generates the session immediately.
                    </p>
            ...
            <GlassCard className="overflow-hidden p-0">
                ...
                <div className="px-6 py-5">
                    <Callout title="LumaPay Sponsors the Gas" tone="orange">
                        In the dynamic flow, LumaPay covers the transaction fees for creating the invoice on-chain. 
                        This means your merchant wallet does not need to hold NIGHT to create new invoices.
                    </Callout>
                    <p className="my-4 text-sm text-gray-400">
                        The SDK will enter a complex polling loop, waiting for the Midnight network to reach consensus on your new invoice. 
            ...
            <GlassCard className="p-6">
                ...
                <div className="mt-4 rounded-lg border border-white/[0.08] bg-white/[0.02] p-4">
                    ...
                    <ul className="space-y-2 text-xs text-gray-400">
                        ...
                        <li>• <b className="text-white">PROCESSING</b>: The buyer has submitted the transaction. Waiting for Midnight confirmations.</li>
