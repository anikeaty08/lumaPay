...
if (!process.env.LUMAPAY_SECRET_KEY) {
    throw new Error('FATAL: LUMAPAY_SECRET_KEY is missing from environment.');
}

const LumaPay = new LumaPay({
    secretKey: process.env.LUMAPAY_SECRET_KEY,
    projectRoot: __dirname // Ensures lumapay.json is found in the same folder
});
...
app.get('/api/verify-payment', async (req, res) => {
    ...
    try {
        const session = await LumaPay.checkout.sessions.retrieve(id);
...
export const backendExampleSection: DocsSection = {
    ...
    summary:
        'The reference backend serves as the blueprint for production-grade LumaPay integrations. It covers initialization, routing, and verification patterns using modern Express.js practices.',
    content: (
        <div className="space-y-6">
            <div className="grid gap-5 md:grid-cols-4">
                ...
                <MetricCard
                    ...
                    description="The SDK handles temporary Midnight network timeouts automatically with an internal exponential backoff."
                />
                <MetricCard
                    icon={FileCode}
            ...
            <GlassCard className="p-6 border-blue-500/20 bg-blue-500/5">
                ...
                <ul className="space-y-3 text-sm text-gray-400">
                    ...
                    <li className="flex gap-2">
                        <span className="text-blue-400 font-bold">•</span>
                        <span><b>Memory Leaks:</b> Always instantiate the <code className="text-white/80">LumaPay</code> class once as a singleton. Do not create a new instance inside every request handler.</span>
                    </li>
                    <li className="flex gap-2">
                        <span className="text-blue-400 font-bold">•</span>
            ...
            <Callout title="Security Checklist" tone="blue">
                ...
                1. <code className="text-blue-200">LUMAPAY_SECRET_KEY</code> is not hardcoded.
