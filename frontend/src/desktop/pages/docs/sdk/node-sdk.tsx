...
const installExample = `# Install the official LumaPay Node.js SDK
npm install @lumapay/node@latest

# Or with yarn:
yarn add @lumapay/node@latest

# For Python environments (FastAPI/Flask/Django):
pip install lumapay-python`;

const configExample = `// The SDK is used via the LumaPay class. Initialize it once per server process.
// Typically in a file like: lib/LumaPay.ts or utils/LumaPay.ts
...
const LumaPay = new LumaPay({
    ...
    baseURL: process.env.LUMAPAY_BASE_URL
          || 'https://api.lumapay.xyz',

    // OPTIONAL: directory where lumapay.json lives (defaults to process.cwd())
    // Essential for Vercel/Next.js where process.cwd() might point to a root
    // directory instead of your function's directory.
    projectRoot: __dirname,

    // OPTIONAL: override the exact path to lumapay.json
...
const sdkApiSurfaceExample = `// Top-level API surface of the LumaPay class:
// The class is structured into logical namespaces for clarity.

const LumaPay = new LumaPay(config);

/**
 * ── Invoice Helpers (Local & Synchronous) ──
 * These methods read from the local lumapay.json manifest.
 * They are extremely fast as they do not perform network requests.
 */
LumaPay.invoices.getAll()                   // → LumaPayInvoice[] (full list)
LumaPay.invoices.getByIndex(0)              // → LumaPayInvoice (by array position)
LumaPay.invoices.getByName('pro-plan')      // → LumaPayInvoice (by dev-assigned name)
LumaPay.invoices.getByType('multipay')      // → LumaPayInvoice[] (filtered by type)

/**
 * ── Checkout Sessions (Remote & Asynchronous) ──
 * These methods interact with the LumaPay REST API and the Midnight network.
 */
LumaPay.checkout.sessions.create(params)    // → Promise<CheckoutSession>
LumaPay.checkout.sessions.retrieve(id)      // → Promise<CheckoutSession>

/**
 * ── Webhook Security (Cryptographic) ──
 * Verification helpers for fulfillment endpoints.
 */
LumaPay.webhooks.verifySignature(payload, sig) // → boolean (Timing-safe HMAC check)
LumaPay.webhooks.constructEvent(payload, sig)  // → WebhookEvent (Verified & Parsed)`;
...
export const nodeSdkSection: DocsSection = {
    ...
    summary:
        'The @lumapay/node SDK is the industrial-grade bridge between your backend and the Midnight blockchain. It abstracts the complexity of ZK-proof generation, relayer sponsorship, and blockchain polling into a clean, developer-friendly interface.',
    content: (
        <div className="space-y-6">
            <div className="grid gap-5 md:grid-cols-4">
                ...
                <MetricCard
                    ...
                    description="Deterministic file resolution logic ensures lumapay.json loads correctly in Vercel, Netlify, and AWS Lambda."
                />
            </div>
            ...
            <GlassCard className="p-6">
                ...
                <div className="mt-4 rounded-lg border border-white/[0.08] bg-white/[0.02] p-4">
                    ...
                    <ul className="mt-2 space-y-2 text-xs text-gray-400">
                        <li>• <code className="rounded bg-white/5 px-1 py-0.5">LumaPay</code>: The primary controller class for all operations.</li>
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-4 text-sm text-gray-400">
                    Traditional \`process.cwd()\` resolution often fails in serverless environments because the working directory varies. The LumaPay SDK provides two explicit controls to solve this:
                </p>
                <CodeBlock title="lib/LumaPay.ts" language="js" code={configExample} />
                <div className="mt-4 grid gap-4 md:grid-cols-2">
                    <div className="rounded-lg border border-blue-500/20 bg-blue-500/5 p-4">
                        <p className="mb-1 text-xs font-bold text-blue-300 uppercase">When to use projectRoot</p>
                        <p className="text-xs text-gray-400">Set this to <code className="text-gray-300">__dirname</code> when your <code className="text-gray-300">lumapay.json</code> is in the same folder as your SDK initialization script.</p>
                    </div>
            ...
            <GlassCard className="p-6">
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-3">
                    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4 transition-all hover:bg-white/[0.04]">
                        ...
                        <p className="text-xs leading-relaxed text-gray-400">Handles local manifest reading. It validates that your <code className="text-white/80">lumapay.json</code> is syntactically correct and prevents hard-to-debug "invoice not found" runtime errors.</p>
                    </div>
                    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4 transition-all hover:bg-white/[0.04]">
                        <div className="mb-2 flex items-center gap-2">
                        ...
                        <p className="text-xs leading-relaxed text-gray-400">The core engine. It manages session creation, polls the Midnight blockchain for mapping resolution (up to 2 minutes with exponential backoff), and returns secure payment URLs.</p>
                    </div>
                    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4 transition-all hover:bg-white/[0.04]">
                        <div className="mb-2 flex items-center gap-2">
                        ...
                        <p className="text-xs leading-relaxed text-gray-400">The gatekeeper. It performs bitwise comparison of signatures to prevent side-channel attacks and ensures that only legitimate LumaPay events trigger your fulfillment logic.</p>
                    </div>
                </div>
            </GlassCard>

            <Callout title="Production Best Practice: Environment Variables" tone="blue">
                Always store your <code className="rounded bg-blue-500/20 px-1.5 py-0.5 text-blue-200">LUMAPAY_SECRET_KEY</code> in a secure environment variable. In your backend, ensure you are reading it correctly using <code className="text-blue-200">process.env.LUMAPAY_SECRET_KEY</code> before passing it to the constructor.
            </Callout>
        </div>
    ),
};
