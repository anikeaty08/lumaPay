...
const mcpToolsExample = `// LumaPay MCP exposes the following tools to AI clients:
...
// User says: "Create a 25 NIGHT invoice for a consulting session"
// Agent calls:
{
  "tool": "mcp_lumapay_create_invoice",
  "params": {
    "amount": 25,
    "currency": "NIGHT",
...
LUMAPAY_MAIN_PASSWORD=your-password         // Optional: for encrypted wallets

// When the model calls login() with no args, the MCP server
// reads LUMAPAY_MAIN_ADDRESS and LUMAPAY_MAIN_PASSWORD from env.
// Wallet approvals stay local to your Midnight wallet/operator process.`;
...
// Tool: mcp_lumapay_login with wallet_preference
{
  "tool": "mcp_lumapay_login",
  "params": {
...
export const mcpIntegrationSection: DocsSection = {
    ...
    title: 'LumaPay MCP — payments in AI clients',
    summary:
        'The LumaPay MCP server is a Model Context Protocol implementation that exposes invoice creation, payment, and transaction inspection as structured tools to AI clients like Claude, Codex, OpenClaw, and Cursor. Wallet approvals stay local; the model decides, but never holds secrets.',
    content: (
        <div className="space-y-6">
            <div className="grid gap-5 md:grid-cols-3">
                ...
                <MetricCard
                    icon={Lock}
                    title="Wallet approval stays local"
                    description="The MCP server process uses local wallet/operator configuration. The AI model never receives or stores recovery phrases or raw key material; it only calls structured tool functions."
                />
                <MetricCard
                    icon={Sparkles}
            ...
            <GlassCard className="p-6">
                <h3 className="mb-4 text-xl font-bold text-white">How LumaPay MCP works</h3>
                <p className="mb-3 text-sm leading-relaxed text-gray-400">
                    When you run <code className="rounded bg-white/5 px-1.5 py-0.5 text-orange-300">npx -y @lumapay/mcp server</code>, the MCP process starts a local stdio-based server. Your AI client (e.g., Claude Desktop) connects to it automatically based on the entry in its MCP servers config. From that point, the AI model can call LumaPay tools in response to your conversational requests.
                </p>
                <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4">
                        ...
                        <ul className="space-y-1 text-xs leading-relaxed text-gray-400">
                            <li>• Wallet recovery phrases or raw private keys</li>
            ...
            <CodeBlock title="Install LumaPay MCP" language="bash" code={installMcpCommand} />
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-3 text-sm leading-relaxed text-gray-400">
                    The <code className="rounded bg-white/5 px-1.5 py-0.5 text-orange-300">mcp_lumapay_create_invoice</code> tool supports all three LumaPay invoice types through the <code className="rounded bg-white/5 px-1.5 py-0.5">invoice_type</code> parameter:
                </p>
                <div className="grid gap-4 md:grid-cols-3">
                    {[
                        ...
                        { type: 'donation', desc: 'Open amount NIGHT donation. Payer decides the contribution size.' },
                    ].map(({ type, desc }) => (
            ...
            <Callout title="LumaPay MCP currency policy" tone="blue">
                The active Preprod MCP flow uses <code className="rounded bg-white/10 px-1.5 py-0.5">currency: "NIGHT"</code>. Stablecoin and ANY-token paths are disabled until those token contracts are available on Midnight Preprod.
            </Callout>

            <Callout title="MCP clients are not merchants" tone="orange">
                LumaPay MCP is designed for the operator or user side — creating invoices, paying links, checking transactions. It is not a replacement for the backend Node SDK. Merchant registration, webhook handling, and server-side session management still belong in your backend using <code className="rounded bg-white/10 px-1.5 py-0.5">@lumapay/node</code>.
            </Callout>
        </div>
    ),
};
