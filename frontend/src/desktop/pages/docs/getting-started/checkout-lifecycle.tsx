...
// session.checkout_url = "https://lumapay.app/checkout/ses_abc123"
...
const verified = await LumaPay.checkout.sessions.retrieve('ses_abc123');
...
// SETTLED  → Payment confirmed on Midnight; safe to fulfill
...
app.post('/api/webhook', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  try {
    ...
    if (event.status === 'SETTLED') {
      // Access event.id (session ID) and event.tx_id (Midnight tx hash)
...
export const checkoutLifecycleSection: DocsSection = {
    ...
    summary:
        'Every LumaPay payment flows through a hosted checkout session. Understanding the lifecycle — creation, buyer payment, settlement, verification, and fulfillment — is the most important thing to get right in any integration.',
    content: (
        <div className="space-y-6">
                    ...
                    {[
                        {
                            ...
                            desc: 'Your server calls LumaPay.checkout.sessions.create() with the payment parameters. The LumaPay backend registers a new session and returns a checkout_url.',
                        },
                        {
                            step: '02',
                            title: 'Buyer is redirected to the hosted checkout',
                            desc: 'Your server redirects the buyer to the checkout_url. This is a LumaPay-hosted page where the buyer connects their wallet, reviews the payment, and submits the Midnight transaction. Your backend is not involved in this step.',
                        },
                        {
                            step: '03',
                            title: 'Midnight transaction is submitted and confirmed',
                            desc: 'The buyer\'s wallet generates a zero-knowledge proof and submits the private transfer and invoice payment to the Midnight network. The session status moves from PENDING to SETTLED once confirmed.',
                        },
                        {
                            step: '04',
                            title: 'Buyer is redirected to success_url',
                            desc: 'LumaPay appends the session_id to your success_url and redirects the buyer back to your app. This redirect alone is not proof of payment — always verify the session status on the server.',
                        },
                        {
                            step: '05',
                        ...
                        },
                        {
                            ...
                            desc: 'If you configured a webhook_url, LumaPay also sends a signed POST with the settled event. Verify the x-lumapay-signature header before acting. This handles async cases where the buyer closes the window early.',
                        },
                    ].map(({ step, title, desc }) => (
            ...
            <div className="grid gap-5 md:grid-cols-4">
                ...
                <MetricCard
                    icon={CheckCircle}
                    title="SETTLED"
                    description="Payment confirmed on the Midnight network. The merchant can safely fulfill the order."
                />
                <MetricCard
                    icon={XCircle}
                    title="FAILED"
                    description="The payment transaction was rejected by the Midnight network."
                />
                <MetricCard
                    icon={Activity}
                    title="EXPIRED"
