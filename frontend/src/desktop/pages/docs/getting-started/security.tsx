...
app.post('/api/webhook/lumapay', express.raw({ type: 'application/json' }), (req, res) => {
  const signature = req.headers['x-lumapay-signature'];

  try {
    const event = LumaPay.webhooks.constructEvent(req.body, signature);
        ...
        <div className="space-y-6">
            <div className="grid gap-5 md:grid-cols-4">
                <MetricCard
                    ...
                    description="LUMAPAY_SECRET_KEY authenticates your merchant backend to the LumaPay API. Never expose it in frontend code, browser contexts, or AI model prompts."
                />
                <MetricCard
                    icon={Lock}
                    title="Private key → Local only"
                    description="Your Midnight wallet private key is managed by Shield (browser) or the MCP process (terminal). It never leaves your machine. Encrypted pass-through never reveals it."
                />
                <MetricCard
                    icon={Server}
                    title="Webhook signature → Server only"
                    description="Always verify x-lumapay-signature header server-side before fulfilling orders. Client-side verification is meaningless and trivially bypassable."
                />
                <MetricCard
                    icon={Shield}
                    ...
                    {[
                        {
                            ...
                            good: 'Store in environment variables on your backend server (e.g., process.env.LUMAPAY_SECRET_KEY). Use it only inside server routes that create or retrieve sessions. Rotate via the merchant console if compromised.',
                        ...
                        {
                            boundary: 'Boundary 2: Midnight private key',
                            good: 'The Shield browser extension and the LumaPay MCP server both keep the private key local. For MCP, pass it via the LUMAPAY_MAIN_PRIVATE_KEY environment variable in the MCP server config — not in the chat prompt. The MCP process performs proof construction locally.',
                        ...
                        },
                        {
                            ...
                            good: 'Call LumaPay.webhooks.constructEvent(rawBody, signature) before processing any webhook payload. This validates the HMAC-SHA256 signature that LumaPay sends with every event. Use express.raw() middleware for the webhook route specifically.',
                            bad: 'Skipping signature verification and trusting the raw POST body. Any attacker who knows your webhook URL could forge a SETTLED event and trigger fulfillment. The signature uses your secret key as the HMAC secret — only your backend and LumaPay can produce valid signatures.',
                        },
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-3 text-sm leading-relaxed text-gray-400">
                    The LumaPay smart contracts enforce security at the protocol level using Midnight's zero-knowledge proof system
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="text-gray-300">
                            ...
                            <tr>
                                ...
                                <td className="px-4 py-3 text-sm text-gray-400">Cannot pay with an inactive token type; active Preprod settlement is NIGHT.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-3 text-sm leading-relaxed text-gray-400">
                    LumaPay's privacy model is based on Midnight's zero-knowledge proof system, which allows transactions to be verified without revealing participants or amounts. Combined with the hash-based invoice design, this creates a multi-layer privacy architecture:
                </p>
                <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-3 rounded-lg border border-white/[0.08] bg-white/[0.02] p-5">
                        <p className="text-sm font-bold text-white">What LumaPay hides (on-chain)</p>
                        <ul className="space-y-2 text-xs leading-relaxed text-gray-400">
                            <li>• Merchant's exact Midnight address at payment time</li>
                    ...
                    <div className="space-y-3 rounded-lg border border-white/[0.08] bg-white/[0.02] p-5">
                        ...
                        <ul className="space-y-2 text-xs leading-relaxed text-gray-400">
                            ...
                            <li>• The transaction fee (in NIGHT)</li>
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-3 text-sm leading-relaxed text-gray-400">
                    Invoice metadata stored in the LumaPay backend (merchant addresses, payer info, notes) is encrypted using
                ...
                <div className="grid gap-4 md:grid-cols-3">
                    ...
                    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4">
                        ...
                        <p className="text-xs leading-relaxed text-gray-400">NIGHT reporting supports selective disclosure — merchants can generate audit reports that reveal only the needed data for compliance without exposing their full payment history.</p>
                    </div>
                    <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4">
            ...
            <GlassCard className="p-6">
                ...
                <p className="mb-3 text-sm leading-relaxed text-gray-400">
                    For users who want maximum privacy as payers, LumaPay's burner wallet support provides full identity separation backed by on-chain encryption:
                </p>
                <ul className="space-y-3 text-sm leading-relaxed text-gray-400">
                    <li className="flex gap-3">
                        ...
                        The burner wallet is generated locally using crypto.getRandomValues() — no link to the main Midnight identity
                    </li>
                    <li className="flex gap-3">
                        <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-orange-300" />
                    ...
                    <li className="flex gap-3">
                        <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-orange-300" />
                        LumaPay MCP supports switching between main and burner wallet mid-session for anonymous agent operations
                    </li>
                </ul>
            </GlassCard>

            <Callout title="Fee sponsoring and Midnight wallet" tone="blue">
                LumaPay supports a Midnight Wallet flow where the LumaPay backend can sponsor the transaction fee on behalf
