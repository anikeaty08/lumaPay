...
import { installCliCommand, LumaPayJsonExample } from '../examples';

const cliWizardStepsExample = `// The CLI wizard prompts you for the following, step by step:
//
// 1. "Connect or configure your Midnight wallet"
...
//    → The LumaPay backend API key from your merchant registration.
//
...
//        - Currency             (NIGHT on Preprod)
//        - Label                (optional human-readable description)
//
// 4. The CLI generates a unique salt per invoice, computes the BHP256 hash,
//    submits the create_invoice transaction to the Midnight testnet via the
//    LumaPay relayer, and waits for confirmation.
//
// 5. On success, writes lumapay.json to your project root.`;

const LumaPayJsonUsageExample = `// After CLI onboarding, your backend can reference invoices by name
// instead of carrying every invoice field in code.

const LumaPay = new LumaPay({
  secretKey: process.env.LUMAPAY_SECRET_KEY,
  projectRoot: __dirname,
  configPath: path.join(__dirname, 'lumapay.json'), // ← CLI output
});
...
for (const invoice of LumaPay.invoices.getAll()) {
...
const relayerNoteExample = `// The LumaPay Relayer is an intermediary that submits
// your on-chain invoice creation transaction on your behalf.
//
// Why it exists:
//   - Creating an invoice on Midnight requires a zero-knowledge proof
//   - Generating this proof client-side for CLI onboarding
//     would require a local Midnight prover (large, slow)
//   - The relayer handles proving and submission for you
//
// What the relayer does NOT have access to:
//   - Your wallet recovery phrase or raw private keys
...
export const cliOnboardingSection: DocsSection = {
    ...
    title: 'CLI onboarding and lumapay.json',
    summary:
        'The LumaPay CLI removes the complexity of manual salt generation, BHP256 hash derivation, and on-chain transaction submission for invoice setup. One interactive wizard run writes a lumapay.json manifest your backend SDK can use directly by invoice name.',
    content: (
        <div className="space-y-6">
            <div className="grid gap-5 md:grid-cols-4">
                ...
                <MetricCard
                    ...
                    description="A cryptographically random salt is generated per invoice. These salts are embedded in lumapay.json and used to verify payments later."
                />
                <MetricCard
                    icon={Search}
                    title="3. Invoices go on-chain"
                    description="The CLI submits NIGHT invoice or campaign creation to Midnight Preprod via the LumaPay relayer/operator."
                />
                <MetricCard
                    icon={Package}
                    title="4. lumapay.json is written"
                    description="The confirmed hash and salt for each invoice are saved locally. Your backend SDK reads this file automatically when lumapay_invoice_name is passed to sessions.create()."
                />
            </div>
            ...
            <GlassCard className="p-6">
                <h3 className="mb-4 text-xl font-bold text-white">Understanding lumapay.json</h3>
                <p className="mb-3 text-sm leading-relaxed text-gray-400">
                    The <code className="rounded bg-white/5 px-1.5 py-0.5 text-orange-300">lumapay.json</code> file is your merchant's invoice manifest. It is a plain JSON file you should commit to source control (it contains no secrets — only hashes and salts, which are public-facing).
                </p>
                <div className="grid gap-4 md:grid-cols-2 mb-5">
                    ...
                    <div className="rounded-lg border border-orange-500/20 bg-orange-500/5 p-4">
                        ...
                        <ul className="mt-2 space-y-1 text-xs leading-relaxed text-gray-400">
                            <li>• <code className="rounded bg-white/5 px-1 py-0.5">LUMAPAY_SECRET_KEY</code> (env variable)</li>
                            <li>• Wallet recovery phrases or raw private keys</li>
                            <li>• Any webhook signing secrets</li>
                        </ul>
                    </div>
                </div>
                <CodeBlock title="lumapay.json structure" language="json" code={LumaPayJsonExample} />
            </GlassCard>

            <CodeBlock title="Using lumapay.json in your backend" language="js" code={LumaPayJsonUsageExample} />

            <GlassCard className="p-6">
                <h3 className="mb-4 text-xl font-bold text-white">The LumaPay Relayer</h3>
                <CodeBlock title="How the relayer works" language="text" code={relayerNoteExample} />
            </GlassCard>

            <Callout title="When to skip lumapay.json" tone="blue">
            ...
            <Callout title="Serverless and Vercel deployments" tone="emerald">
                If your backend runs on a serverless platform like Vercel, pass <code className="rounded bg-white/10 px-1.5 py-0.5">projectRoot: __dirname</code> and an absolute <code className="rounded bg-white/10 px-1.5 py-0.5">configPath</code> to the LumaPay constructor. Otherwise the SDK may resolve <code className="rounded bg-white/10 px-1.5 py-0.5">lumapay.json</code> relative to the build output directory rather than your source root.
            </Callout>
        </div>
    ),
};
