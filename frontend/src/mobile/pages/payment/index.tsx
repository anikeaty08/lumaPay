...
import { LumaPayCardPaymentPanel } from '../../../shared/components/payments/LumaPayCardPaymentPanel';
...
import { getUtf8ByteLength, COMPACT_PAYMENT_NOTE_MAX_BYTES } from '../../../shared/utils/core/compactInputLimits';
import { looksLikeMidnightAddress, normalizeMidnightAddress } from '../../../shared/utils/midnight/midnightAddress';

const MobilePaymentPage = () => {
    const getTokenCodeFromType = (_tokenType: number): TokenCode => 'NIGHT';
    ...
    const processPaymentLink = (rawValue: string) => {
        ...
        try {
            ...
            if ((urlObj.searchParams.get('merchant') && urlObj.searchParams.get('salt')) || urlObj.searchParams.get('hash')) {
                ...
                console.warn("Invalid LumaPay Link");
            }
        } catch (e) {
    ...
    const isMultiPay = programId === 'campaign';
    ...
    const payerNoteTooLong = payerNoteBytes > COMPACT_PAYMENT_NOTE_MAX_BYTES;
    const merchantNoteTooLong = merchantNoteBytes > COMPACT_PAYMENT_NOTE_MAX_BYTES;
    const normalizedGiftCardPayerAddress = normalizeMidnightAddress(giftCardPayerAddress);
    const giftCardPayerAddressInvalid = normalizedGiftCardPayerAddress.length > 0 && !looksLikeMidnightAddress(normalizedGiftCardPayerAddress);
                ...
                >
                    <div className="text-center mb-6">
                        <h1 className="text-3xl font-bold mb-2 tracking-tighter text-white">
                            Scan <span className="text-transparent bg-clip-text bg-gradient-to-r from-neon-primary to-neon-accent">Private Invoice</span>
                        </h1>
                        <p className="text-sm text-gray-400">Point your camera at a LumaPay QR Code</p>
                    </div>
                    ...
                    <div className="w-full space-y-3">
                        <Input
                            placeholder="Paste LumaPay Link"
                            value={manualLink}
            ...
            >
                ...
                <div className="text-center mb-6">
                        <h1 className="text-3xl font-bold mb-4 tracking-tighter text-white">
                            {step === 'SUCCESS' ? 'Private Payment' : step === 'ALREADY_PAID' ? 'Private Invoice' : 'Make'} <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-300 via-orange-500 to-amber-400 drop-shadow-[0_0_20px_rgba(249,115,22,0.22)]">{step === 'SUCCESS' ? 'Successful' : step === 'ALREADY_PAID' ? 'Paid' : 'Private Payment'}</span>
                        </h1>
                        ...
                        {(step === 'SUCCESS' || step === 'ALREADY_PAID') ? (
                            <div className="text-center space-y-4">
                                ...
                                {txId && (
                                    <Button
                                        variant="primary"
                                        onClick={() => window.open(`https://api-preprod.1am.xyz/api/v4/graphql#transaction-${txId}`, '_blank')}
                                    >
                                        View Transaction
                                    </Button>
                                )}
                                ...
                                <div className="rounded-2xl bg-black/25 p-4 space-y-4 mb-4">
                                    <div>
                                        ...
                                        <p className={`mt-2 text-[11px] ${payerNoteTooLong ? 'text-red-400' : 'text-gray-500'}`}>
                                            Payer note: {payerNoteBytes}/{COMPACT_PAYMENT_NOTE_MAX_BYTES} bytes.
                                        </p>
                                    </div>
                                        ...
                                        {shareMerchantNote && (
                                            ...
                                            />
                                                <p className={`px-1 text-[11px] ${merchantNoteTooLong ? 'text-red-400' : 'text-gray-500'}`}>
                                                    Merchant note: {merchantNoteBytes}/{COMPACT_PAYMENT_NOTE_MAX_BYTES} bytes.
                                                </p>
                                            </motion.div>
                                        )}
                                    </div>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <p className={`px-1 text-[10px] leading-relaxed ${giftCardPayerAddressInvalid ? 'text-red-400' : 'text-gray-500'}`}>
                                                {giftCardPayerAddressInvalid
                                                    ? 'Please enter a valid Midnight public address.'
